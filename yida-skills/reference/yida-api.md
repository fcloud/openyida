...
### getFormNavigationListByOrder（获取应用导航菜单列表）

- **描述**：获取宜搭应用的完整导航菜单列表，包含所有表单、自定义页面、报表等页面信息。可用于查询报表的 `topicId`、页面的 `formUuid`、页面类型等元信息。
- **地址**：`GET /dingtalk/web/{appType}/query/formnav/getFormNavigationListByOrder.json`
- **参数**：

| 参数 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| `_api` | String | 是 | 固定 `Nav.queryList` |
| `_mock` | String | 是 | 固定 `false` |
| `_csrf_token` | String | 是 | CSRF Token（由 yida-login 获取） |
| `_locale_time_zone_offset` | String | 否 | 时区偏移（毫秒），如 `28800000`（东八区） |
| `_stamp` | String | 否 | 请求时间戳（毫秒） |
  ...
  "content": [
    {
      ...
      "formType": "receipt",
      "navType": "PAGE",
      "title": { "zh_CN": "表单名称", "en_US": "Form Name", "type": "i18n" },
      "topicId": null,
      "hidden": "n",
      "mobileHidden": null,
      "formStatus": "ONLINE",
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `formUuid` | String | 页面 UUID（如 `FORM-XXX`、`REPORT-XXX`） |
| `formType` | String | 页面类型：`receipt`=表单、`display`=自定义页面、`report`=报表 |
| `navType` | String | 导航类型：`SYSTEM`=系统内置菜单、`PAGE`=用户创建的页面 |
| `title` | Object | 页面标题（i18n 格式），通过 `title.zh_CN` 获取中文名 |
| `topicId` | Number | 报表数据源 ID（仅 `formType=report` 时有值，同一应用下的报表共享同一个 topicId） |
| `hidden` | String | 是否在导航中隐藏：`y`=隐藏、`n`=显示 |
| `mobileHidden` | String | 是否在移动端隐藏：`y`=隐藏 |
| `formStatus` | String | 页面状态：`ONLINE`=已上线 |
| `listOrder` | Number | 导航排序序号（从 0 开始） |
| `navUuid` | String | 导航项 UUID |
| `parentNavUuid` | String | 父级导航 UUID |
| `parentId` | Number | 父级 ID（0 表示顶级） |
| `displayType` | String | 展示类型：`RE`=自定义页面渲染模式，空字符串=默认 |
| `relateFormUuid` | String | 关联的表单/页面 UUID |
| `processCode` | String | 流程编码（仅流程表单有值） |
| `isNew` | String | 是否新建：`y`/`n` |
| `isNewForm` | String | 是否新建表单：`y`/`n` |
| `isNewReport` | String | 是否新建报表：`y`/`n` |

- **典型用途**：

| 场景 | 过滤条件 | 获取字段 |
| --- | --- | --- |
| 获取报表的 topicId | `formType === 'report'` | `topicId` |
| 获取所有表单页面 | `formType === 'receipt'` | `formUuid`、`title.zh_CN` |
| 获取所有自定义页面 | `formType === 'display'` | `formUuid`、`title.zh_CN` |
| 获取所有可见页面 | `hidden === 'n' && navType === 'PAGE'` | 全部字段 |
...
const appType = 'APP_XXXXX';
const csrfToken = 'your-csrf-token';
const cookie = 'your-cookie-string';

const url = `https://www.aliwork.com/dingtalk/web/${appType}/query/formnav/getFormNavigationListByOrder.json`
...
if (result.success) {
  const navList = result.content;

  // 示例：查找报表的 topicId
  const reportFormUuid = 'REPORT-XXX';
  const reportNav = navList.find(item => item.formUuid === reportFormUuid);
  if (reportNav) {
    console.log('topicId:', reportNav.topicId);
  }

  // 示例：列出所有可见的用户页面
  const visiblePages = navList.filter(item => item.hidden === 'n' && item.navType === 'PAGE');
  visiblePages.forEach(page => {
    console.log(`${page.title.zh_CN} (${page.formType}) - ${page.formUuid}`);
  });
}
```
