...
1. 不要在移动端页面提供密度切换 UI，移动端固定使用 `spacious`
2. 不要硬编码像素值而不使用 `DENSITY_CONFIG` 变量，否则密度切换失效
3. 不要在固定展示的报表/大屏页面提供密度切换，直接用 `spacious`
4. 不要在表单填写页提供密度切换，固定使用 `comfortable`

### 重要规则（IMPORTANT）

1. 每次生成自定义页面时都必须考虑密度，无需用户明确提及
2. 列表/表格类页面必须默认提供密度切换 UI
3. 在 `didMount` 中必须检测移动端并自动降级为 `spacious`
4. 所有间距、字体、行高必须从 `DENSITY_CONFIG[density]` 读取，不得硬编码
5. **本技能不读写 memory**：密度配置作为页面代码的一部分写入宜搭平台，不依赖跨会话的 memory 状态
...
> 用户明确指定时优先使用用户选择（“紧凑/密集”→ compact，“宽松/大字体”→ spacious），未指定时根据上表场景自动选择。
...
完整 `DENSITY_CONFIG` 配置（compact / comfortable / spacious 三套完整样式变量）详见下方参考文档。
...
| 文档 | 覆盖范围 | 何时阅读 |
|------|---------|--------|
| [密度配置参考](references/density-config.md) | compact / comfortable / spacious 三套完整样式变量定义 | 编写密度相关代码时 |
