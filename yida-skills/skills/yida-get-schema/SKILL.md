...
### 致命规则（FATAL）
违反会导致功能失败或运行时报错：
1. **绝对禁止猜测或编造 fieldId**，宜搭字段 ID 由平台随机生成（格式如 `textField_eftt1aa5m`、`selectField_fix024y92`），无法从字段名称推断，必须通过此命令获取
2. **不要在未获取 Schema 的情况下执行任何涉及字段 ID 的操作**
3. **不要假设字段 ID 格式**，即使看起来像 `textField_xxx`，也必须通过命令确认
4. **不要跳过 Schema 获取步骤直接进行数据操作**，即使用户催促也必须先获取
5. **不要在 Schema 获取失败时继续执行后续操作**，必须先解决问题
6. **不要缓存过期的 Schema 信息**，表单结构变更后必须重新获取

### 重要规则（IMPORTANT）
影响代码质量和用户体验：
1. **凡是需要用到字段 ID（fieldId）的操作，必须先执行此命令**，不得跳过
2. **将关键字段 ID 映射（字段名 → fieldId）记录到 `.cache/<项目名>-schema.json`**，供后续操作复用
3. **录入/更新数据后，必须用 `openyida data query --size 1` 抽查一条记录**，确认 `formData` 中字段有实际值（非空 `""`），若全部为空说明字段 ID 有误，需重新排查
