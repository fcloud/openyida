...
2. **不要用此命令创建表单页面** — 带字段的数据收集页应使用 `yida-create-form-page`

### 重要规则（IMPORTANT）

影响代码质量和用户体验：

1. **创建前必须确认** — 执行创建命令前必须向用户确认页面名称和目标应用
2. **创建成功后记录 formUuid** — 写入 `.cache/<项目名>-schema.json`
3. **创建后必须继续流程** — 执行 `yida-custom-page` 编写 JSX 代码，再用 `yida-publish-page` 发布
4. **本技能不读写 memory** — formUuid 等信息输出到 stdout，通过 `.cache/<项目名>-schema.json` 持久化
