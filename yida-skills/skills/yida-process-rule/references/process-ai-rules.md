...
以下三种情况必须识别为回退场景并配置 `routeRules`：

1. **关键词触发**：当节点名称、条件分支名称或流程描述中出现「驳回」「退回」「返回修改」「不同意」「重新提交」等关键词时，识别为回退场景，跳转目标为当前节点的前序提交/处理节点
2. **条件分支末尾回环**：当条件分支中有「不合格」「不通过」分支，且该分支末尾节点语义上需要回到前序节点时，必须配置跳转规则，目标为产生问题的检查/处理节点
3. **显式循环要求**：当用户描述中明确提到「循环审批」「反复处理直到通过」「多次提交」等需求时，配置拒绝时跳转回起始处理节点
