...
### 致命规则（FATAL）
违反会导致功能失败或运行时报错：
1. **不要在非 Claude Code 环境下省略 `--input` 参数**（其他环境无法自动检测对话文件）

### 重要规则（IMPORTANT）
影响代码质量和用户体验：
1. **导出前确认当前环境**（`openyida env`），Claude Code 环境可自动检测，其他环境需 `--input` 手动指定
2. **导出前必须确认输出路径有写权限**，避免导出失败
3. **导出完成后必须告知用户文件路径和文件大小**，方便用户确认
4. **遇到对话文件格式不支持时，必须明确告知用户支持的格式**（`.jsonl`）
5. **本技能不读写 memory**：对话记录导出为本地 Markdown 文件，不依赖跨会话的 memory 状态
