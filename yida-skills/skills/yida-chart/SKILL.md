---
name: yida-chart
description: "宜搭 ECharts 高级图表 / 高级报表可视化组件技能。通过 ECharts + 自定义页面 JSX 实现高度定制化的数据图表、报表视觉升级和图表交互。本技能不负责完整 Dashboard / 看板 / 驾驶舱的产品化交付；完整看板场景优先使用 yida-dashboard，再按需调用本技能实现具体图表。本技能不负责创建宜搭原生报表（标准报表由 yida-report 技能负责），但 ECharts 报表必须依赖宜搭原生报表的 getDataAsync.json 或 getCacheData.json 接口获取聚合数据，禁止前端聚合。当用户明确提到「ECharts」「echarts」「高级图表」「图表美化」「图表交互」「定制化图表」或用户提供报表 URL 要求做图表级视觉升级时，使用此技能。普通的「报表」「统计」需求默认由 yida-report 处理；完整「看板」「驾驶舱」「数据大屏」「Dashboard」需求默认由 yida-dashboard 处理。"
---
...
- 不要把完整 Dashboard / 看板 / 驾驶舱需求只按本技能处理；这类需求先用 `yida-dashboard` 编排
...
| 用户意图 | 触发条件 |
|---------|---------|
| 高级定制化可视化 | "更美观"、"ECharts"、"高级图表"、"图表交互"、"定制化图表" |
| 优化已有报表 | 用户提供报表 URL 要求优化 |
| 完整 Dashboard / 看板 / 驾驶舱 | → 改用 `yida-dashboard` 技能编排，按需调用本技能 |
...
| 技能 | 定位 | 典型场景 |
|------|------|---------|
| **yida-dashboard**（看板） | 完整经营看板产品化交付 | 「看板」「驾驶舱」「Dashboard」「数据大屏」「截图」「短链」「待办闭环」需求 |
| **yida-report**（原生报表） | 创建宜搭平台内置报表，作为数据源 | 普通「报表」「统计」需求 |
| **yida-chart**（本技能） | 基于原生报表数据，用 ECharts 实现自定义可视化图表 | 「ECharts」「高级图表」「图表美化」「图表交互」「定制化图表」需求 |
...
> 💡 **与 yida-dashboard / yida-report 的分工**：完整"看板"、"驾驶舱"、"Dashboard"、"数据大屏"需求默认由 `yida-dashboard` 编排；普通"报表"、"统计"需求默认由 `yida-report` 处理；只有具体图表需要 ECharts 定制、交互或视觉升级时，才使用本技能。
...
- **用户需要完整看板 / Dashboard / 驾驶舱** → 不直接使用本技能，先使用 `yida-dashboard` 技能
