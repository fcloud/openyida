...
//   selectField_xxx   → 实际的分类字段 ID（用于多系列分组）
...
export function loadTrendData() {
  ...
  this.fetchAllFormData(DATA_FORM_UUID)
    .then(function(dataList) {
      var trendData = this.buildTrendSeries(dataList);
      this.setCustomState({ loading: false, trendData: trendData });
      setTimeout(function() {
 ...
 * @returns {{ months: string[], series: { name: string, data: number[] }[] }}
 */
export function buildTrendSeries(dataList) {
  var seriesMap = {};
  var monthSet = {};
...
export function renderTrendChart() {
  ...
  var seriesList = trendData.series || [];
  ...
  chart.setOption({
    ...
    series: seriesList.map(function(s, index) {
      return {
        ...
        symbol: 'circle',
        symbolSize: 6,
        lineStyle: { width: 2, color: THEME_COLORS[index % THEME_COLORS.length] },
...
export function renderJsx() {
  ...
  return (
    <div>
      <div style={{ display: 'none' }}>{timestamp}</div>
      <div style={styles.container}>
        <div style={styles.header}>趋势分析</div>
