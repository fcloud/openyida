...
1. **不要编造 appType** — 必须从命令返回的 JSON 中提取
2. **不要在未确认 corpId 的情况下创建应用** — 先运行 `openyida env` 确认登录态
3. **不要重复创建同名应用** — 先询问用户是否已有应用

### 重要规则（IMPORTANT）

影响代码质量和用户体验：

1. **创建成功后记录 appType** — 写入 `.cache/<项目名>-schema.json`
2. **创建前确认组织** — 当前登录的组织（corpId）与目标组织一致
3. **本技能不读写 memory** — appType 等信息输出到 stdout，通过 `.cache/<项目名>-schema.json` 持久化
