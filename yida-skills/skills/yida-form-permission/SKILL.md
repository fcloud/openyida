...
1. 不要在未查询现有权限组的情况下直接创建新权限组（先用 get-permission 查询）
2. 不要修改字段权限（暂不支持，需通过宜搭管理后台手动配置）
3. 不要使用自定义部门或自定义过滤条件（不支持）

### 重要规则（IMPORTANT）

1. 修改权限前先用 `openyida get-permission` 查询现有配置
2. action-permission 为完全替换模式，只保留 true 的项，操作前确认影响范围
3. **本技能不读写 memory**：权限配置通过 CLI 命令写入宜搭平台，不依赖跨会话的 memory 状态
...
openyida save-permission <appType> <formUuid> \
  --data-permission '{"dataRange":"SELF"}' \
  --action-permission '{"operations":{"OPERATE_VIEW":true,"OPERATE_EDIT":true}}'
```
