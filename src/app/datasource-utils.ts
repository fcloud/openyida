 ...
 */
function buildConnectorFitFunctionSource(): string {
  return (
 ...
 * @returns urlParams 数据源对象
 */
function buildUrlParamsDataSource(formUuid: string): any {
  return {
    id: '',
    name: 'urlParams',
    description:
 ...
 * @param definition.id - 数据源名称（JS 中通过 this.dataSourceMap.<id> 调用）
 * @param definition.connectorId - 连接器 ID
 * @param definition.actionId - 连接器动作 ID
 * @param formUuid - 表单 UUID
 * @param csrfToken - CSRF Token
 * @returns 完整的连接器数据源 Schema
 */
function buildSingleConnectorDataSource(definition: any, formUuid: string, csrfToken: string): any {
 ...
 *
 * @param datasourceDefinitions - 数据源定义数组
 * @param formUuid - 表单 UUID
 * @param csrfToken - CSRF Token
 * @returns 数据源对象数组（用于写入 dataSource.online 和 dataSource.list）
 */
export function buildDataSourceList(datasourceDefinitions: any[], formUuid?: string, csrfToken?: string): any[] {
 ...
 *
 * @param value - JSON 字符串（以 [ 开头）或 JSON 文件路径
 * @returns 数据源定义数组
 */
export function parseDatasourceArg(value: string | undefined): any[] {
  if (!value) {return [];}
 ...
 */
export function extractDatasourceArg(args: string[]): { datasourceValue: string | null; remainingArgs: string[] } {
  const remainingArgs: string[] = [];
  let datasourceValue: string | null = null;
 ...
 *
 * @param baseUrl - 宜搭 base URL
 * @param connectorId - 连接器 ID
 * @param csrfToken - CSRF Token
 * @param cookies - Cookie 数组
 * @returns connectionId -> connectionName 映射
 */
export async function fetchConnectionNameMap(baseUrl: string, connectorId: string, csrfToken: string, cookies: any[]): Promise<Map<number, string>> {
  const { httpGet } = require('../core/utils');
  try {
    const result: any = await httpGet(baseUrl, '/query/connection/getAliveConnections.json', {
    ...
    const connectionNameMap = new Map<number, string>();
    if (result && result.success && Array.isArray(result.content)) {
      result.content.forEach(function (item: any) {
 ...
 *
 * @param datasourceDefinitions - 数据源定义数组（会原地修改）
 * @param baseUrl - 宜搭 base URL
 * @param csrfToken - CSRF Token
 * @param cookies - Cookie 数组
 */
export async function enrichDatasourceDefinitionsWithConnectionLabels(datasourceDefinitions: any[], baseUrl: string, csrfToken: string, cookies: any[]): Promise<void> {
  ...
  const connectorConnectionCache = new Map<string, Map<number, string>>();
