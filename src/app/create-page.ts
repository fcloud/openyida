...
export async function run(args: string[]): Promise<void> {
  ...
  const datasourceDefinitions = parseDatasourceArg(datasourceValue ?? undefined);
  if (datasourceDefinitions.length > 0) {
    console.error(t('create_page.datasource_injecting', datasourceDefinitions.length.toString()));
