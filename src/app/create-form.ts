 ...
 */
function buildApiPath(appType: string, apiName: string, options: any = {}): string {
...
function readFieldsDefinition(fieldsJsonOrFile: string): { fields: any[], columns: number } {
...
function readChangesDefinition(changesJsonOrFile: string): any[] {
  let rawContent: string;
...
function generateFieldId(componentName: string): string {
...
function i18n(text: string, enText?: string): any {
...
function buildOptionDataSource(options: string[]): any[] {
  return options.map(function (optionText: string, optionIndex: number) {
    return {
...
function buildFieldComponent(field: any): any {
  ...
  if (componentName === 'TableField' && field.children) {
    component.children = field.children.map(function (childField: any) {
...
function collectComponentNames(fields: any[]): string[] {
  ...
  fields.forEach(function (field: any) {
    ...
    if (field.type === 'TableField' && field.children) {
      field.children.forEach(function (child: any) {
...
function buildComponentsMap(componentNames: string[]): any[] {
  return componentNames.map(function (name: string) {
    return {
...
function inferComponentNameFromFieldId(fieldId: string): string {
...
function normalizeFillingRule(rule: any): any {
...
function normalizeFillingRules(fillingRules: any): any {
  ...
  if (Array.isArray(normalized.tableRules)) {
    normalized.tableRules = normalized.tableRules.map(function (tableRule: any) {
...
function resolveFieldIdReferences(fieldComponents: any[]): void {
  // 构建 label → fieldId 的映射表
  const labelToFieldId: Record<string, string> = {};
  fieldComponents.forEach(function (component: any) {
  ...
  fieldComponents.forEach(function (component: any) {
     ...
     */
    function resolveRules(rules: any[]): void {
     ...
     */
    function resolveTableRules(tableRules: any[]): void {
      if (!Array.isArray(tableRules)) {return;}
      tableRules.forEach(function (tableRule: any, tableIndex: number) {
        if (!tableRule.rules || !Array.isArray(tableRule.rules)) {return;}

        console.error(t('create_form.table_filling_rule', String(tableIndex + 1), tableRule.tableId));

        tableRule.rules.forEach(function (rule: any, ruleIndex: number) {
          if (rule.target && typeof rule.target === 'string' && rule.target.startsWith('@label:')) {
            const targetLabel = rule.target.slice(7);
            const resolvedId = labelToFieldId[targetLabel];
            if (resolvedId) {
              console.error(t('create_form.table_rule_resolved', String(ruleIndex + 1), targetLabel, resolvedId));
              rule.target = resolvedId;
            } else {
    ...
    const hasTableRules = fillingRules.tableRules && fillingRules.tableRules.some(function (tr: any) {
 ...
 */
function getLayoutConfig(layout: string): any {
 ...
 */
function getThemeConfig(theme: string, labelAlign: string): any {
  const baseConfig = {
 ...
 */
function groupFieldsByGroup(fields: any[]): any[] {
  const groups: any[] = [];
  ...
  groupMap.forEach((groupFields: any[], groupName: string) => {
 ...
 */
function buildGroupedFieldComponents(fields: any[], formLayout: string): any[] {
  const groups = groupFieldsByGroup(fields);

  return groups.map((group: any, groupIndex: number) => {
    // 构建分组内的字段组件
    const groupFieldComponents = group.fields.map((field: any) => buildFieldComponent(field));
...
function buildFormSchema(
  ...
  const fieldComponents = fields.map(function (field: any) {
...
function sendGetRequest(baseUrl: string, cookies: any[], requestPath: string, queryParams: any): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    request.on('error', (requestError: any) => {
      reject(requestError);
    });
...
function buildEmptyFormSchema(): any {
  const constructorCode = "function constructor() {\nvar module = { exports: {} };\nvar _this = this;\nthis.__initMethods__(module.exports, module);\nObject.keys(module.exports).forEach(function(item) {\n  if(typeof module.exports[item] === 'function'){\n    _this[item] = module.exports[item];\n  }\n});\n\n}";
  const actionsCompiled = '"use strict";\n\nexports.__esModule = true;\nexports.didMount = didMount;\nfunction didMount() {\n  console.log("\\u300C\\u9875\\u9762 JS\\u300D\\uFF1A\\u5F53\\u524D\\u9875\\u9762\\u5730\\u5740 " + location.href);\n}\n';
  const actionsSource = 'export function didMount() {\n  console.log(`「页面 JS」：当前页面地址 ${location.href}`);\n}';
  return {
...
function extractLabelText(component: any): string {
...
function findFormContainer(node: any): any {
...
function findFieldIndexByLabel(fields: any[], label: string): number {
...
function applyFieldChanges(component: any, changes: any): void {
  ...
  if (changes.required !== undefined) {
    if (changes.required) {
      const hasRequired = (props.validation || []).some(function (rule: any) {
      ...
      props.validation = (props.validation || []).filter(function (rule: any) {
        return rule.type !== 'required';
      });
    }
  }
  ...
  if (component.componentName === 'AssociationFormField') {
    ...
    if (changes.dataFillingRules !== undefined) {
      ...
      const hasTableRules = changes.dataFillingRules.tableRules && changes.dataFillingRules.tableRules.some(function (tr: any) {
        return tr.rules && tr.rules.length > 0;
      });
...
function ensureComponentsMap(schema: any, componentName: string): void {
  const pageSchema = schema.pages[0];
  const existingNames = pageSchema.componentsMap.map(function (entry: any) {
...
function applyChangesToSchema(schema: any, changes: any[]): any[] {
  ...
  changes.forEach(function (change: any, changeIndex: number) {
    const actionDesc = t('create_form.action_label', String(changeIndex + 1), change.action);
    ...
    if (change.action === 'add') {
      ...
      if (change.field.type === 'TableField' && change.field.children) {
        change.field.children.forEach(function (childField: any) {
  ...
  function ensureTopLevelFieldId(comps: any[]): void {
    comps.forEach(function (comp: any) {
...
function sendPostRequest(baseUrl: string, csrfToken: string, cookies: any[], requestPath: string, extraParams: any, formUuid?: string): Promise<any> {
  return new Promise((resolve, reject) => {
...
function sendUpdateConfigRequest(baseUrl: string, csrfToken: string, cookies: any[], appType: string, formUuid: string, version: number, value: number): Promise<any> {
  return new Promise((resolve, reject) => {
    const postData = querystring.stringify({
...
async function requestWithAutoLogin(requestFn: (auth: any) => Promise<any>, authRef: any): Promise<any> {
...
async function saveSchemaAndUpdateConfig(authRef: any, appType: string, formUuid: string, schema: any, version?: number, stepOffset?: number): Promise<{ saveResult: any, configResult: any }> {
  const saveStep = stepOffset || 4;
  const configStep = saveStep + 1;

  console.error(t('create_form.step_save_schema', String(saveStep)));
  console.error(t('create_form.sending_save'));

  const saveResult = await requestWithAutoLogin(function (auth: any) {
  ...
  if (version !== undefined) {
    console.error(t('create_form.current_version', String(version)));
  }

  console.error(t('create_form.step_update_config', String(configStep)));
  console.error(t('create_form.sending_config'));

  const configResult = await requestWithAutoLogin(function (auth: any) {
...
async function mainCreate(parsedArgs: any, csrfToken: string, cookies: any[], baseUrl: string, cookieData: any): Promise<void> {
  ...
  console.error(t('create_form.fields_loaded', fields.length.toString()));
  console.error(t('create_form.pc_columns', String(columns)));
  fields.forEach(function (field: any, index: number) {
  ...
  const createResult = await requestWithAutoLogin(function (auth: any) {
    return sendPostRequest(
      auth.baseUrl, auth.csrfToken, auth.cookies,
  ...
  if (datasourceDefinitions.length > 0) {
    console.error(t('create_form.datasource_count', datasourceDefinitions.length.toString()));
...
function fillSerialNumberFormulas(components: any[], corpId: string, appType: string, formUuid: string): void {
...
async function mainUpdate(parsedArgs: any, csrfToken: string, cookies: any[], baseUrl: string, cookieData: any): Promise<void> {
  ...
  const schemaResult = await requestWithAutoLogin(function (auth: any) {
  ...
  if (formContainer && formContainer.children) {
    ...
    formContainer.children.forEach(function (child: any, childIndex: number) {
  ...
  console.error(t('create_form.changes_loaded', changes.length.toString()));
  changes.forEach(function (change: any, changeIndex: number) {
    if (change.action === 'add') {
  ...
  if (configResult && configResult.success) {
    ...
    console.error(t('create_form.form_uuid_label', formUuid));
    console.error(t('create_form.url_label', formUrl));
    console.error(t('create_form.changes_applied', appliedChanges.length.toString()));
    console.error(t('create_form.config_updated_0'));
    console.error(SEP2);
