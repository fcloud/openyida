...
async function createApp(appName: string, authRef: any): Promise<string> {
...
async function createBlankForm(appType: string, formTitle: string, authRef: any): Promise<string> {
  const postData = querystring.stringify({
    _csrf_token: authRef.csrfToken,
    formType: 'receipt',
...
async function saveFormSchema(appType: string, formUuid: string, schema: any, authRef: any): Promise<any> {
  const postData = querystring.stringify({
    _csrf_token: authRef.csrfToken,
    appType,
    formUuid,
...
async function updateFormConfig(appType: string, formUuid: string, authRef: any): Promise<any> {
  const postData = querystring.stringify({
    _csrf_token: authRef.csrfToken,
    appType,
    formUuid,
...
function adaptSerialNumberFormulas(schema: any, oldAppType: string, newAppType: string, oldFormUuid: string, newFormUuid: string): any {
...
function extractSchemaContent(schemaResult: any): any {
...
export async function run(args: string[]): Promise<void> {
  ...
  console.error(t('import.read_ok', forms.length.toString()));
  ...
  console.error(t('import.success_count', successCount.toString()));
  if (failCount > 0) {
    console.error(t('import.fail_count', failCount.toString()));
  }
