 ...
 * @returns 上传结果
 */
async function uploadToOss(ossClient: any, filePath: string, objectKey: string): Promise<any> {
  return new Promise((resolve, reject) => {
    const stream = fs.createReadStream(filePath);
    ossClient.putStream(objectKey, stream, (err: Error, result: any) => {
      if (err) {
        reject(err);
      } else {
        resolve(result);
 ...
 * @returns OSS 客户端实例
 */
function createOssClient(config: any): any {
  try {
 ...
 * @returns 上传结果数组
 */
export async function performUpload(options: ParseArgsResult): Promise<UploadResult[]> {
  ...
  console.log(t('cdn.uploading_images', imageFiles.length.toString()));
  ...
  for (const filePath of imageFiles) {
    ...
    try {
      ...
      console.error(t('cdn.upload_failed', fileName, (error as Error).message));
    }
  }
...
export async function run(args: string[]): Promise<void> {
  ...
  try {
    ...
    console.log(t('cdn.upload_success_count', successCount.toString()));
    if (failCount > 0) {
      console.log(t('cdn.upload_fail_count', failCount.toString()));
    }
    ...
    console.error(t('cdn.upload_error', (error as Error).message));
    process.exit(1);
  }
}
