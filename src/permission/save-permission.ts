...
const DATA_RANGE_TO_PERMIT_TYPE: Record<string, string> = {
  'ALL': 'ALL',
  'SELF': 'SELF',
  'DEPT': 'DEPT',
...
function validateDataPermission(dataPermission: any): void {
...
function validateActionPermission(actionPermission: any): void {
 ...
 * 构建权限组的 roleData 对象
 */
function buildRoleData(permitPackage: any, overrideMembers: string[] | null): any {
  ...
  const nonPersonsEntries = (existingRoleData.include || []).filter(
    (entry: any) => entry.roleType !== 'PERSONS'
  );
  const newInclude: Array<{ roleType: string; roleValue: string }> = [...nonPersonsEntries];
 ...
 * 保存单个权限组（新增或更新）
 */
function savePermitPackage(appType: string, formUuid: string, permitPackage: any, overrideMembers: string[] | null, authRef: AuthRef): Promise<any> {
  return new Promise((resolve) => {
    const { cookies, csrfToken, baseUrl } = authRef;
    const cookieHeader = cookies.map((c) => `${c.name}=${c.value}`).join('; ');
    const parsedUrl = new URL(baseUrl);
...
export async function run(args: string[]): Promise<void> {
  ...
  try {
    ...
    console.error(`  ❌ 参数验证失败: ${(err as Error).message}`);
    process.exit(1);
  }
  ...
  if (createMode) {
    ...
    const newOperatePermit: Record<string, string> = {};
    if (actionPermission) {
  ...
  const packages = ((listResult.content as any) && (listResult.content as any).formPermit) || [];
  ...
  const packagesToUpdate = packages.filter((pkg: any) => {
    if (targetRole === 'DEFAULT') {
      return pkg.roleMembers && pkg.roleMembers.some((rm: any) => rm.roleType === 'DEFAULT');
    }
    if (targetRole === 'MANAGER') {
      return pkg.roleMembers && pkg.roleMembers.some((rm: any) => rm.roleType === 'MANAGER');
    }
    return true;
  });
