...
const OP_CODE_TO_DISPLAY: Record<string, string> = {
  Equal: '等于',
  NotEqual: '不等于',
  Contains: '包含',
  NotContain: '不包含',
...
function i18n(zhText: string, enText?: string): { en_US: string; zh_CN: string; type: string } {
...
function buildAppendActions(): any[] {
  return [
...
function assignNodeIdsRecursive(nodes: any[], nameToIdMap: Record<string, string>): void {
  nodes.forEach(function (node) {
    if (node.type === 'route') {
      node._nodeId = generateNodeId();
      const conditions = node.conditions || [];
      conditions.forEach(function (cond: any) {
...
function buildCarbonNode(node: any, nextNodeId: string): { processNode: any; viewNode: any } {
  const nodeId = node._nodeId;
...
function buildRouteNode(node: any, exitNodeId: string, nameToIdMap: Record<string, string>): { processNodes: any[]; viewNodes: any[] } {
  ...
  const conditionProcessNodes: any[] = [];
  const conditionViewNodes: any[] = [];

  // 构建每个条件分支
  conditions.forEach(function (cond: any, index: number) {
    const condNodeId = cond._nodeId;
    conditionNodeIds.push(condNodeId);

    // 递归构建条件分支内的子节点
    let childProcessNodes: any[] = [];
    let childViewNodes: any[] = [];
    if (cond.childNodes && cond.childNodes.length > 0) {
...
function buildNodeListRecursive(nodes: any[], exitNodeId: string, nameToIdMap: Record<string, string>): { processNodes: any[]; viewNodes: any[] } {
...
function buildProcessAndViewJson(definition: any, processCode: string, formUuid: string, baseUrl: string, appType: string): { processJson: any; viewJson: any } {
...
function queryProcessVersions(authRef: AuthRef, appType: string, processCode: string, status: string): Promise<any> {
...
function getProcessCodeFromAppParam(authRef: AuthRef, appType: string, formUuid: string): Promise<string | null> {
  const requestPath = '/' + appType + '/query/app/getAppPlatFormParam.json'
    ...
    + '&_stamp=' + Date.now();
  return httpGet(authRef.baseUrl, requestPath, null, authRef.cookies).then(function (result: any) {
...
function getProcessCodeFromSchema(authRef: AuthRef, appType: string, formUuid: string): Promise<string | null> {
...
export async function run(args: string[]): Promise<void> {
  ...
  if (!isSimpleFormat && !isFullFormat) {
    console.error('  ❌ ' + t('configure_process.invalid_format'));
  ...
  let processCode: string | null = processCodeArg;
  ...
  if (draftResult.success && draftResult.content && draftResult.content.processId) {
    ...
    if (savedResult.success && savedResult.content && savedResult.content.data) {
      const savedVersions = savedResult.content.data.filter(function (item: any) { return item.status === 'SAVED'; });
