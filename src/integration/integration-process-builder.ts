 ...
 */
function mapEventTypes(events: string[]): string[] {
 ...
 * 构建获取单条数据节点的过滤条件对象
 */
function buildDataRetrieveCondition(dataConditions: Array<{
  bFieldId: string;
  bFieldName: string;
  aFieldId: string;
 ...
 * 构建新增数据节点的字段赋值列表
 */
function buildDataCreateAssignments(assignments: Array<{
  column: string;
  valueType: string;
  value: string;
}>): any[] {
