import {
  ...
  generateRuleGroupId,
  generateDataRuleId,
} from './integration-node-ids';
import { buildDataRetrieveCondition } from './integration-process-builder';
 ...
 */
function buildAddDataChildList(components: any[]): any[] {
  return components
...
function buildViewJson(options: {
  ...
  toUsers: Array<{ userId: string; userName: string }>;
  appType: string;
  nodeIds: string[];
  addDataFormUuid?: string;
  addDataAssignments?: Array<{ column: string; valueType: string; value: string }>;
  addDataFormSchema?: any[];
  addDataFormName?: string;
  dataFormUuid?: string;
  ...
  if (hasAddDataNode && addDataNodeId) {
    ...
    const componentOptionMap: Record<string, string> = {};
...
export { buildAddDataChildList, buildViewJson };
