import { loadCookieData, triggerLogin, resolveBaseUrl } from '../core/utils';
import { generateNodeId } from './integration-node-ids';
import { getFormSchema, saveProcess, createLogicflow } from './integration-api';
import { mapEventTypes, buildProcessJson } from './integration-process-builder';
import { buildViewJson } from './integration-view-builder';
 ...
 */
function parseFlag(args: string[], flagName: string): string | null {
...
async function run(args: string[]): Promise<void> {
  ...
  const addDataAssignments: Array<{
    column: string;
    valueType: string;
    value: string;
  }> = [];
  ...
  if (!processCode) {
    ...
    try {
      ...
      console.error(t('integration.create_new_flow_failed', (error as Error).message));
      console.error(SEP);
  ...
  const processNodeIds: string[] = [triggerNodeId];
  ...
  const viewNodeIds: string[] = [canvasId, triggerNodeId];
  ...
  if (addDataFormUuid) {
    try {
      ...
      console.error(t('integration.create_get_schema_warn', (error as Error).message));
    }
  }
