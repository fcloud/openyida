...
function parseArgs(): ParseParamsResult {
...
function validateParams(params: ParseParamsResult): void {
...
async function main(): Promise<void> {
  ...
  try {
    validateParams({ openUrl, isOpen, openAuth } as any);
    console.error(t('save_share_config.validate_ok'));
  } catch (err) {
    console.error(t('save_share_config.validate_failed', (err as Error).message));
    process.exit(1);
  }
  ...
  function buildPostData(token: string): string {
