 ...
 */
function genFieldId(componentName: string): string {
  return componentName + '_' + randomId();
}

// ── 基础组件列表 ──────────────────────────────────────

const BASE_COMPONENTS: string[] = [
 ...
 */
function inferDataType(fieldType: string, explicitDataType?: string): string {
  if (explicitDataType) {
 ...
 */
function deriveFilterFieldCode(fieldId: string, fieldType?: string): string {
