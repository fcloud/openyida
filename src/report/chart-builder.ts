 ...
 */
function normalizeCubeCode(code: string): string {
  if (!code) {return '';}
 ...
 */
function buildDataViewQueryModel(chart: any, cubeTenantId: string): any {
  ...
  if (Array.isArray(chart.fields) && chart.fields.length > 0 && !chart.xField && !chart.yField) {
    ...
    if (chart.xField) {
      if (Array.isArray(chart.xField)) {
        chart.xField.forEach((f: any) => allFields.push({ ...f, role: 'x' }));
      } else {
    ...
    if (Array.isArray(chart.yField)) {
      chart.yField.forEach((f: any) => allFields.push({ ...f, role: 'y' }));
    ...
    if (chart.groupField) {
      if (Array.isArray(chart.groupField)) {
        chart.groupField.forEach((f: any) => allFields.push({ ...f, role: 'group' }));
      } else {
 ...
 */
function buildDataSetModelMap(chart: any, cubeTenantId: string): any {
  const cubeCode = chart.cubeCode || '';
  ...
  if (chartType === 'table') {
    ...
    if (Array.isArray(chart.columnFields)) {
      chart.columnFields.forEach((f: any) => allFields.push({ ...f, role: 'col' }));
    } else if (Array.isArray(chart.columns)) {
      chart.columns.forEach((f: any) => allFields.push({ ...f, role: 'col' }));
    } else if (Array.isArray(chart.fields)) {
      chart.fields.forEach((f: any) => allFields.push({ ...f, role: 'col' }));
    }
  ...
  if (chartType === 'indicator') {
    ...
    if (Array.isArray(chart.kpi)) {
      chart.kpi.forEach((f: any) => allFields.push({ ...f, role: 'kpi' }));
    } else if (Array.isArray(chart.yField)) {
      chart.yField.forEach((f: any) => allFields.push({ ...f, role: 'kpi' }));
    } else if (Array.isArray(chart.fields)) {
      chart.fields.forEach((f: any) => allFields.push({ ...f, role: 'kpi' }));
    }
    if (Array.isArray(chart.helpKpi)) {
      chart.helpKpi.forEach((f: any) => allFields.push({ ...f, role: 'helpKpi' }));
    }
  ...
  if (chartType === 'pivot') {
    ...
    if (Array.isArray(chart.columnList)) {
      chart.columnList.forEach((f: any) => allFields.push({ ...f, role: 'col' }));
    } else if (Array.isArray(chart.columns)) {
      chart.columns.forEach((f: any) => allFields.push({ ...f, role: 'col' }));
    }
  ...
  const fieldListObjs = allFields.map((f: any) =>
    buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType)
  );
  const xFieldObjs = allFields
    .filter((f: any) => f.role === 'x')
    .map((f: any) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
  const yFieldObjs = allFields
    .filter((f: any) => f.role === 'y')
    .map((f: any) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
  const groupFieldObjs = allFields
    .filter((f: any) => f.role === 'group')
    .map((f: any) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
...
function buildBarChartSettings(): any {
  return {
...
function buildLineChartSettings(): any {
  return {
    container: { height: 248 },
    style: {
...
function buildPieChartSettings(): any {
  return {
    container: { height: 248 },
    style: {
...
function buildScatterChartSettings(): any {
  return {
    container: { height: 248 },
    style: {
...
function buildAreaChartSettings(): any {
  const s = buildLineChartSettings();
  s.style.showArea = true;
  return s;
}

function buildFunnelChartSettings(): any {
  return {
    container: { height: 248 },
    style: {
      colorType: 'SCHEMA_COLOR', chartColorsMode: 'defaultColorsMode',
...
function buildRadarChartSettings(): any {
  return {
    container: { height: 248 },
    style: {
      colorType: 'SCHEMA_COLOR', chartColorsMode: 'defaultColorsMode',
...
function buildGaugeChartSettings(): any {
  return {
    container: { height: 248 },
...
function buildTableSettings(): any {
  return {
...
function buildComboChartSettings(): any {
  return {
    container: { height: 248 },
    style: {
...
function buildIndicatorSettings(): any {
  return {
...
function buildPivotSettings(): any {
  return {
    rglConfig: { w: 6, h: 21, isHeightAuto: true },
...
function buildNumberChartSettings(): any {
  return {
 ...
 * 根据图表类型获取 settings
 */
function getChartSettings(chartType: string): any {
  switch (chartType) {
...
function buildUserConfig(chartType: string): any {
  if (chartType === 'pie') {
 ...
 */
function buildUserConfigWithFields(chartType: string): any {
  // 指标卡已经是正确的数组格式
  if (chartType === 'indicator') {
...
function buildMockData(chartType: string): any[] {
  if (chartType === 'bar') {
    return [{
      name: 'chartData',
      data: {
 ...
 * 构建完整的报表 Schema
 */
function buildReportSchema(reportTitle: string, charts: any[], reportId?: string, cubeTenantId?: string): any {
  ...
  const componentsMap = Array.from(usedComponentNames).map((name) => {
    const entry: any = { package: '@/components/vc-yida-report', version: '1.0.6', componentName: name };
 ...
 */
function buildFilterFieldListItem(
  cubeCode: string,
  alias: string,
  aliasNameZh: string,
  fieldCode: string,
 ...
 */
function buildSelectFilter(filterDef: any, filterNodeId: string, filterFieldId: string): any {
 ...
 */
function buildFilterContainer(filterComponents: any[], containerFieldId: string): any {
