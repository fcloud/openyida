 ...
 */
function generateActionInfo(path: string, method: string): ActionInfo {
 ...
 * 生成执行动作配置
 */
function generateOperation(curlData: CurlData, relevantHeaders: [string, string][]): Operation {
  ...
  if (queryInput && queryInput.childList) {
    parameters.query = queryInput.childList.map((child: any) => ({
      name: child.name,
      value: ''
    }));
  }
