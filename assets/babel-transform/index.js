...
function ownKeys(object, enumerableOnly) { const keys = Object.keys(object); if (Object.getOwnPropertySymbols) {
...
function _toPrimitive(input, hint) { if (_typeof(input) !== 'object' || input === null) {
    return input;
} const prim = input[Symbol.toPrimitive]; if (prim !== undefined) {
...
function findComps(ast) {
    ...
    const _ref = (typeof window === 'undefined' ? 'undefined' : _typeof(window)) === 'object' ? window : {}, MyBabel = _ref.MyBabel;
    if (!ast || !MyBabel) {
        return;
    }
    const comps = [];
        ...
        try {
            ...
            if (parseFloat(env.RE_VERSION, 10) < 7.0) {
                ...
                let plugins = [function (_ref2) {
                        const types = _ref2.types, template = _ref2.template;
