...
exports.buildAfterFetch = buildAfterFetch;
exports.buildExportData = buildExportData;
exports.buildLink = buildLink;
exports.buildFieldObj = buildFieldObj;
exports.buildDataViewQueryModel = buildDataViewQueryModel;
exports.buildDataSetModelMap = buildDataSetModelMap;
exports.buildUserConfig = buildUserConfig;
exports.buildUserConfigWithFields = buildUserConfigWithFields;
exports.buildMockData = buildMockData;
exports.getChartSettings = getChartSettings;
exports.buildReportSchema = buildReportSchema;
exports.buildSelectFilter = buildSelectFilter;
exports.buildFilterContainer = buildFilterContainer;
exports.buildFilterFieldListItem = buildFilterFieldListItem;
exports.injectFilterLinkage = injectFilterLinkage;
...
function buildDataViewQueryModel(chart, cubeTenantId) {
    ...
    allFields.forEach((f) => {
        const alias = (0, constants_1.genFieldAlias)();
        f._alias = alias;
...
function buildDataSetModelMap(chart, cubeTenantId) {
    ...
    if (chartType === 'combo') {
        ...
        const fieldListObjs = allFields.map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        const xFieldObjs = allFields.filter((f) => f.role === 'x').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        const leftYObjs = allFields.filter((f) => f.role === 'leftY').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        const rightYObjs = allFields.filter((f) => f.role === 'rightY').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        return {
            dataSetName: {
                dataViewQueryModel: {
    ...
    if (chartType === 'indicator') {
        ...
        const fieldListObjs = allFields.map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        const kpiObjs = allFields.filter((f) => f.role === 'kpi').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        const helpKpiObjs = allFields.filter((f) => f.role === 'helpKpi').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType, f.aggregateType));
        return {
            youshuData: {
                dataViewQueryModel: {
                    cubeCode, fieldDefinitionList, fieldList: fieldListKeys,
    ...
    if (chartType === 'gauge') {
        const allFields = [];
        if (chart.valueField) {
            allFields.push({ ...chart.valueField, role: 'value' });
        }
        if (chart.assitValueField) {
            allFields.push({ ...chart.assitValueField, role: 'assit' });
        }
        ...
        const fieldListObjs = allFields.map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType || 'DOUBLE', f.aggregateType || 'AVG'));
        const valueFieldObjs = allFields.filter((f) => f.role === 'value').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType || 'DOUBLE', f.aggregateType || 'AVG'));
        const assitValueFieldObjs = allFields.filter((f) => f.role === 'assit').map((f) => buildFieldObj(cubeCode, f.fieldCode, f.aliasName || f.fieldCode, f._alias, f.dataType || 'DOUBLE', f.aggregateType || 'AVG'));
        return {
            chartData: {
                dataViewQueryModel: {
                    cubeCode, fieldDefinitionList, fieldList: fieldListKeys,
...
function buildUserConfigWithFields(chartType) {
    ...
    if (['bar', 'line', 'area', 'scatter', 'radar', 'funnel'].includes(chartType)) {
        ...
        if (chartType !== 'funnel') {
            items.push({ setterName: 'ColumnFieldSetter', name: 'groupField', title: '分组',
                setterProps: { single: true, showFormatTab: true, showEditTab: true } }, { setterName: 'ColumnFieldSetter', name: 'annotationField', title: '参考线',
                setterProps: { showFormatTab: true, showEditTab: true } });
        }
...
function buildReportSchema(reportTitle, charts, reportId, cubeTenantId) {
    const pageNodeId = (0, constants_1.genNodeId)();
    const rootHeaderId = (0, constants_1.genNodeId)();
    const pageHeaderId = (0, constants_1.genNodeId)();
    const pageHeaderContentId = (0, constants_1.genNodeId)();
    const rootContentId = (0, constants_1.genNodeId)();
    const rootFooterId = (0, constants_1.genNodeId)();
    ...
    const usedComponentNames = new Set(constants_1.BASE_COMPONENTS);
    charts.forEach((chart) => {
    ...
    charts.forEach((chart, index) => {
        ...
        const userConfig = buildUserConfigWithFields(chart.type);
        chartChildren.push({
    ...
    return {
                ...
                componentsTree: [
                    {
                        ...
                        props: {
                            ...
                            className: 'page_' + (0, constants_1.randomId)(),
                        },
                        dataSource: {
                            offline: [],
                            globalConfig: {
                                ...
                                children: [
                                    {
                                        ...
                                        children: [
                                            ...
                                            { componentName: 'PageHeaderTab', id: (0, constants_1.genNodeId)(), props: {}, children: [] },
                                        ],
                                    },
                                ],
                            },
        ...
        id: reportId || ('REPORT-' + (0, constants_1.randomId)().toUpperCase() + (0, constants_1.randomId)().toUpperCase()),
 ...
 * @param filterNodeId 筛选器节点 ID
 * @param filterFieldId 筛选器 fieldId
 */
function buildSelectFilter(filterDef, filterNodeId, filterFieldId) {
    ...
    const aliasValue = 'field_filter_value_' + (0, constants_1.randomId)().slice(0, 4);
    const aliasLabel = 'field_filter_label_' + (0, constants_1.randomId)().slice(0, 4);
    const valueFieldDef = filterDef.valueField || {};
    const labelFieldDef = filterDef.labelField || filterDef.valueField || {};
    const valueFieldItem = buildFilterFieldListItem(cubeCode, aliasValue, valueFieldDef.aliasName || '查询字段', valueFieldDef.fieldCode || '', valueFieldDef.dataType || 'STRING');
    const labelFieldItem = buildFilterFieldListItem(cubeCode, aliasLabel, labelFieldDef.aliasName || '显示字段', labelFieldDef.fieldCode || valueFieldDef.fieldCode || '', labelFieldDef.dataType || 'STRING');
 ...
 *
 * @param filterComponents buildSelectFilter 返回的筛选器组件数组
 * @param containerFieldId 容器 fieldId
 */
function buildFilterContainer(filterComponents, containerFieldId) {
    ...
    return {
        ...
        id: 'node_filter_container_' + (0, constants_1.randomId)(),
        props: {
            rglSwitch: true,
            createForm: true,
            status: 'normal',
            ...
            cid: 'node_filter_container_' + (0, constants_1.randomId)(),
            layout: layout,
        },
 ...
 *
 * @param dataSetModelMap 图表的 dataSetModelMap
 * @param filterMeta buildSelectFilter 返回的 __filterMeta__
 * @param filterFieldCode 被筛选的字段 fieldCode（在图表数据源中）
 * @param cubeCode 数据源 cubeCode
 * @param cubeTenantId 租户 ID
 */
function injectFilterLinkage(dataSetModelMap, filterMeta, filterFieldCode, cubeCode, cubeTenantId) {
    ...
    const filterKey = 'filter-' + (0, constants_1.randomId)() + '-' + (0, constants_1.randomId)() + '-' + (0, constants_1.randomId)() + '-' + (0, constants_1.randomId)();
    // paramId 格式："{筛选器nodeId}-selectFilter"
    const paramId = filterNodeId + '-selectFilter';
    // 筛选引用字段的 alias（额外添加到 fieldDefinitionList）
    const aliasFilterRef = 'field_filter_ref_' + (0, constants_1.randomId)().slice(0, 4);
    ...
    if (ds.dataViewQueryModel) {
        if (!ds.dataViewQueryModel.filterList) {
            ds.dataViewQueryModel.filterList = [];
        }
