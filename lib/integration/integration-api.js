...
exports.createLogicflow = createLogicflow;
exports.listLogicflows = listLogicflows;
exports.switchLogicflow = switchLogicflow;
...
async function getFormSchema(authRef, params) {
    ...
    const response = await (0, utils_1.requestWithAutoLogin)((auth) => {
        return (0, utils_1.httpGet)(auth.baseUrl, `/alibaba/web/${appType}/_view/query/formdesign/getFormSchema.json?formUuid=${formUuid}&schemaVersion=V5`, null, auth.cookies);
    }, authRef);
...
async function saveProcess(authRef, params) {
    ...
    return (0, utils_1.requestWithAutoLogin)((auth) => {
...
async function createLogicflow(authRef, params) {
    ...
    const response = await (0, utils_1.requestWithAutoLogin)((auth) => {
        ...
        return (0, utils_1.httpPost)(auth.baseUrl, `/alibaba/web/${appType}/query/formLogicflowBinding/createLogicflow.json`, postData, auth.cookies);
    }, authRef);
    if (!response || !response.success) {
        const errorMsg = response ? response.errorMsg || JSON.stringify(response) : '请求失败';
...
async function listLogicflows(authRef, params) {
    const { appType, key = '', formUuid = '', status = '', pageIndex = 1, pageSize = 10, } = params;
    const response = await (0, utils_1.requestWithAutoLogin)((auth) => {
        ...
        return (0, utils_1.httpGet)(auth.baseUrl, `/alibaba/web/${appType}/query/appLogicflowBinding/listflow.json?${query}`, null, auth.cookies);
    }, authRef);
    if (!response || !response.success) {
        const errorMsg = response ? response.errorMsg || JSON.stringify(response) : '请求失败';
...
async function switchLogicflow(authRef, params) {
    ...
    const response = await (0, utils_1.requestWithAutoLogin)((auth) => {
        ...
        return (0, utils_1.httpPost)(auth.baseUrl, `/alibaba/web/${appType}/query/formLogicflowBinding/switchflow.json?_api=Connector.switchFlow&_mock=false&_stamp=${stamp}`, postData, auth.cookies);
    }, authRef);
    if (!response || !response.success) {
        const errorMsg = response ? response.errorMsg || JSON.stringify(response) : '请求失败';
