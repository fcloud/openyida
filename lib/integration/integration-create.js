...
async function run(args) {
  ...
  if (dataFormUuid) {
    ...
    console.error(t('integration.create_data_conditions', dataConditions.length));
  }
  ...
  const step = (label) => {
    currentStep++;
    console.error(t('integration.create_step', currentStep, totalSteps, label));
  };
  ...
  if (addDataFormUuid) {
    try {
      ...
      console.error(t('integration.create_get_schema_ok', addDataFormSchema.length));
    } catch (error) {
