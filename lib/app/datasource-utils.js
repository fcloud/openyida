...
exports.buildDataSourceList = buildDataSourceList;
exports.parseDatasourceArg = parseDatasourceArg;
exports.extractDatasourceArg = extractDatasourceArg;
exports.fetchConnectionNameMap = fetchConnectionNameMap;
exports.enrichDatasourceDefinitionsWithConnectionLabels = enrichDatasourceDefinitionsWithConnectionLabels;
/**
 * 构建连接器数据源的 options.fit 函数源码字符串（宜搭连接器专用格式）。
 * 连接器返回值会多包一层，需要解析 serviceReturnValue。
 */
function buildConnectorFitFunctionSource() {
    return ('function fit(response) {\n' +
...
function buildSingleConnectorDataSource(definition, formUuid, csrfToken) {
    ...
    const fitCompiled = 'function main(){\n' +
        '    \n' +
        "    'use strict';\n" +
        '\n' +
    ...
    const dataHandlerSource = 'function(data, err) { this.setState({' + dataSourceName + ': data}); return data; }';
    return {
        id: '',
        name: dataSourceName,
        description: '',
...
async function enrichDatasourceDefinitionsWithConnectionLabels(datasourceDefinitions, baseUrl, csrfToken, cookies) {
    if (!datasourceDefinitions || datasourceDefinitions.length === 0) {
        return;
    }
    ...
    const queryResults = await Promise.all(connectorIdList.map(function (connectorId) {
