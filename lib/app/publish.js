...
// @ts-ignore
const babel_transform_1 = __importDefault(require("../core/babel-transform"));
// @ts-ignore
const UglifyJS = __importStar(require("uglify-js"));
const utils_1 = require("../core/utils");
const i18n_1 = require("../core/i18n");
// ── 配置读取 ──────────────────────────────────────────
const CONFIG = fs.existsSync(path.resolve((0, utils_1.findProjectRoot)(), 'config.json'))
    ? JSON.parse(fs.readFileSync(path.resolve((0, utils_1.findProjectRoot)(), 'config.json'), 'utf-8'))
    : {};
...
function parseArgs() {
    ...
    if (args.length < 3) {
        console.error((0, i18n_1.t)('publish.usage'));
        console.error((0, i18n_1.t)('publish.example'));
        process.exit(1);
    }
    return {
        appType: args[0],
...
function compileSource(sourcePath) {
    ...
    const compiledPath = path.join((0, utils_1.findProjectRoot)(), 'pages', 'dist', compiledFileName);
    console.error((0, i18n_1.t)('publish.reading_source', sourceFileName));
    const sourceCode = fs.readFileSync(sourcePath, 'utf-8');
    console.error((0, i18n_1.t)('publish.compiling', sourceFileName));
    const babelResult = (0, babel_transform_1.default)(sourceCode, {}, false, { RE_VERSION: '7.4.0' });
    if (babelResult.error instanceof Error) {
        const err = babelResult.error;
        let errorMsg = (0, i18n_1.t)('publish.compile_failed', err.message);
        if (err.loc) {
            errorMsg += (0, i18n_1.t)('publish.compile_location', err.loc.line, err.loc.column);
        }
        if (err.code) {
            errorMsg += (0, i18n_1.t)('publish.compile_error_code', err.code);
        }
        // 提供常见错误排查提示
        const hintLines = [
            '',
    ...
    console.error((0, i18n_1.t)('publish.minifying', compiledFileName));
    const uglifyResult = UglifyJS.minify(babelResult.compiled);
    if (uglifyResult.error) {
        console.error((0, i18n_1.t)('publish.minify_failed', uglifyResult.error.message));
        process.exit(1);
    }
    // 确保输出目录存在
    ...
    console.error((0, i18n_1.t)('publish.compile_done', compiledPath));
...
function buildSchemaContent(sourceCode, compiledCode, formUuid) {
    console.error((0, i18n_1.t)('publish.building_schema'));
    // 构造函数代码（固定模板）
...
function sendSaveRequest(csrfToken, cookies, schemaContent, baseUrl, appType, formUuid) {
    return new Promise((resolve, reject) => {
        ...
        const request = requestModule.request(requestOptions, (response) => {
            ...
            response.on('end', () => {
                console.error((0, i18n_1.t)('common.http_status', response.statusCode));
                let parsed;
                try {
...
async function main() {
    ...
    if (!fs.existsSync(sourcePath)) {
        console.error((0, i18n_1.t)('publish.source_not_found', sourcePath));
        process.exit(1);
    }
    const parsedSource = path.parse(sourcePath);
    const compiledPath = path.join((0, utils_1.findProjectRoot)(), 'pages', 'dist', `${parsedSource.name}.js`);
    // Step 1: 编译源码 + 构建 Schema
    console.error((0, i18n_1.t)('publish.step_compile'));
    ...
    console.error((0, i18n_1.t)('publish.schema_built'));
    // Step 2: 读取登录态（优先从本地缓存 Cookie 中提取 csrf_token）
    console.error((0, i18n_1.t)('common.step_login', '2'));
    ...
    const SEP = '='.repeat(50);
    console.error(SEP);
    console.error((0, i18n_1.t)('publish.title'));
    console.error(SEP);
    console.error((0, i18n_1.t)('publish.base_url', baseUrl));
    console.error((0, i18n_1.t)('publish.app_type', appType));
    console.error((0, i18n_1.t)('publish.form_uuid', formUuid));
    console.error((0, i18n_1.t)('publish.source_file', sourcePath));
    console.error((0, i18n_1.t)('publish.compiled_file', compiledPath));
    // Step 3: 发布 Schema（307 时刷新 csrf_token，302 时自动重登录，均自动重试）
    console.error((0, i18n_1.t)('publish.step_publish'));
    ...
    if (response && response.__csrfExpired) {
        ...
        console.error((0, i18n_1.t)('publish.resend_save_csrf'));
    ...
    if (response && response.__needLogin) {
        ...
        cookies = cookieData.cookies;
        baseUrl = resolveBaseUrl(cookieData);
        console.error((0, i18n_1.t)('publish.resend_save'));
        response = await sendSaveRequest(csrfToken, cookies, schemaContent, baseUrl, appType, formUuid);
    }
    if (!response || !response.success) {
        const errorMsg = response ? response.errorMsg || (0, i18n_1.t)('common.unknown_error') : (0, i18n_1.t)('common.request_failed');
        console.error((0, i18n_1.t)('publish.publish_failed', errorMsg));
        if (response && !response.__needLogin && !response.__csrfExpired) {
            console.error((0, i18n_1.t)('common.response_detail', JSON.stringify(response, null, 2)));
        }
        process.exit(1);
    }
    ...
    console.error((0, i18n_1.t)('publish.schema_published'));
    console.error((0, i18n_1.t)('publish.form_uuid_label', savedFormUuid));
    console.error((0, i18n_1.t)('publish.version_label', version));
    // Step 4: 更新表单配置（307 时刷新 csrf_token，302 时自动重登录，均自动重试）
    console.error((0, i18n_1.t)('publish.step_config'));
    console.error((0, i18n_1.t)('publish.sending_config'));
    ...
    if (configResponse && configResponse.__csrfExpired) {
        ...
        cookies = cookieData.cookies;
        baseUrl = resolveBaseUrl(cookieData);
        console.error((0, i18n_1.t)('publish.resend_config_csrf'));
    ...
    if (configResponse && configResponse.__needLogin) {
        ...
        cookies = cookieData.cookies;
        baseUrl = resolveBaseUrl(cookieData);
        console.error((0, i18n_1.t)('publish.resend_config'));
        configResponse = await sendUpdateConfigRequest(csrfToken, cookies, baseUrl, appType, savedFormUuid, version, 8);
    }
    // 输出结果
    ...
    if (configResponse && configResponse.success) {
        console.error((0, i18n_1.t)('publish.success'));
        console.error((0, i18n_1.t)('publish.form_uuid_label', savedFormUuid));
        console.error((0, i18n_1.t)('publish.version_label', version));
        console.error((0, i18n_1.t)('publish.config_updated'));
    }
    else {
        const errorMsg = configResponse ? configResponse.errorMsg || (0, i18n_1.t)('common.unknown_error') : (0, i18n_1.t)('common.request_failed');
        console.error((0, i18n_1.t)('publish.config_failed', errorMsg));
        console.error((0, i18n_1.t)('publish.schema_ok_config_failed'));
        if (configResponse && !configResponse.__needLogin && !configResponse.__csrfExpired) {
            console.error((0, i18n_1.t)('common.response_detail', JSON.stringify(configResponse, null, 2)));
        }
    }
    console.error(SEP2);
}
...
if (require.main === module || process.argv[1]?.includes('publish')) {
    main().catch((error) => {
        console.error((0, i18n_1.t)('publish.exception', error.message));
        process.exit(1);
    });
}
