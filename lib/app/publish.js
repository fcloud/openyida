...
function normalizeShortUrlSegment(value) {
  return String(value || '')
    .trim()
    .replace(/\.[^.]+$/, '')
    .replace(/[^a-zA-Z0-9_-]+/g, '-')
    .replace(/^-+|-+$/g, '')
    .substring(0, 48) || 'dashboard';
}

function buildShortUrlCandidates(sourcePath, formUuid) {
  const baseName = normalizeShortUrlSegment(path.parse(sourcePath).name);
  const formSuffix = normalizeShortUrlSegment(String(formUuid || '').replace(/^FORM-?/i, '').slice(-8).toLowerCase());
  return [
    `/s/${baseName}`,
    `/s/${baseName}-${formSuffix}`,
  ];
}

function sendVerifyShortUrlRequest(csrfToken, cookies, baseUrl, appType, formUuid, shareUrl) {
  return new Promise((resolve, reject) => {
    ...
    const requestPath = `/dingtalk/web/${appType}/query/formdesign/verifyShortUrl.json?${queryParams}`;
...
function sendSaveShareConfigRequest(csrfToken, cookies, baseUrl, appType, formUuid, shareUrl) {
  return new Promise((resolve, reject) => {
    const postData = querystring.stringify({
    ...
    const requestOptions = {
      hostname: parsedUrl.hostname,
      port: parsedUrl.port || (isHttps ? 443 : 80),
      path: `/dingtalk/web/${appType}/query/formdesign/saveShareConfig.json`,
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Content-Length': Buffer.byteLength(postData),
...
function sendGetShareConfigRequest(csrfToken, cookies, baseUrl, appType, formUuid) {
  return new Promise((resolve, reject) => {
    const postData = querystring.stringify({
    ...
    const requestOptions = {
      hostname: parsedUrl.hostname,
      port: parsedUrl.port || (isHttps ? 443 : 80),
      path: `/dingtalk/web/${appType}/query/formdesign/getShareConfig.json`,
      method: 'POST',
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Content-Length': Buffer.byteLength(postData),
...
async function createDefaultShareUrl(authState, appType, formUuid, sourcePath) {
  let currentConfig = await sendGetShareConfigRequest(authState.csrfToken, authState.cookies, authState.baseUrl, appType, formUuid);

  if (currentConfig && currentConfig.__csrfExpired) {
    const cookieData = refreshCsrfToken();
    authState.csrfToken = cookieData.csrf_token;
    authState.cookies = cookieData.cookies;
    authState.baseUrl = resolveBaseUrl(cookieData);
    currentConfig = await sendGetShareConfigRequest(authState.csrfToken, authState.cookies, authState.baseUrl, appType, formUuid);
  }

  if (currentConfig && currentConfig.__needLogin) {
  ...
  const existingShareUrl = currentConfig && currentConfig.content && currentConfig.content.shareUrl;
  if (existingShareUrl) {
    return { shareUrl: existingShareUrl, fullShareUrl: authState.baseUrl + existingShareUrl, reused: true };
  }

  const candidates = buildShortUrlCandidates(sourcePath, formUuid);
  let lastError = '';

  for (const shareUrl of candidates) {
    let verifyResponse = await sendVerifyShortUrlRequest(
      authState.csrfToken,
      authState.cookies,
      authState.baseUrl,
      appType,
    ...
    if (verifyResponse && verifyResponse.__csrfExpired) {
      const cookieData = refreshCsrfToken();
      authState.csrfToken = cookieData.csrf_token;
      authState.cookies = cookieData.cookies;
      authState.baseUrl = resolveBaseUrl(cookieData);
      verifyResponse = await sendVerifyShortUrlRequest(authState.csrfToken, authState.cookies, authState.baseUrl, appType, formUuid, shareUrl);
    }

    if (verifyResponse && verifyResponse.__needLogin) {
      const cookieData = triggerLogin();
      authState.csrfToken = cookieData.csrf_token;
      authState.cookies = cookieData.cookies;
      authState.baseUrl = resolveBaseUrl(cookieData);
    ...
    if (!verifyResponse || !verifyResponse.success || !verifyResponse.content) {
      lastError = verifyResponse ? verifyResponse.errorMsg || t('common.unknown_error') : t('common.request_failed');
      continue;
    }

    let saveResponse = await sendSaveShareConfigRequest(authState.csrfToken, authState.cookies, authState.baseUrl, appType, formUuid, shareUrl);

    if (saveResponse && saveResponse.__csrfExpired) {
      const cookieData = refreshCsrfToken();
      authState.csrfToken = cookieData.csrf_token;
      authState.cookies = cookieData.cookies;
      authState.baseUrl = resolveBaseUrl(cookieData);
      saveResponse = await sendSaveShareConfigRequest(authState.csrfToken, authState.cookies, authState.baseUrl, appType, formUuid, shareUrl);
    }

    if (saveResponse && saveResponse.__needLogin) {
      const cookieData = triggerLogin();
      authState.csrfToken = cookieData.csrf_token;
      authState.cookies = cookieData.cookies;
      authState.baseUrl = resolveBaseUrl(cookieData);
    ...
    if (saveResponse && saveResponse.success) {
      return { shareUrl, fullShareUrl: authState.baseUrl + shareUrl };
    }

    lastError = saveResponse ? saveResponse.errorMsg || t('common.unknown_error') : t('common.request_failed');
  }

  return { warning: lastError || t('common.unknown_error') };
}
...
async function main() {
  ...
  const authState = { csrfToken, cookies, baseUrl };
  const shareResult = await createDefaultShareUrl(authState, appType, savedFormUuid, sourcePath);
  csrfToken = authState.csrfToken;
  cookies = authState.cookies;
  baseUrl = authState.baseUrl;
  const adminUrl = `${baseUrl}/${appType}/admin/${savedFormUuid}`;
  const appAdminUrl = `${baseUrl}/${appType}/admin`;
  const resultRows = [
    ['Form UUID', savedFormUuid],
    ['Version', String(version)],
    ['URL', pageUrl],
    ['App Admin URL', appAdminUrl],
    ['Page Admin URL', adminUrl],
  ];
  if (shareResult.fullShareUrl) {
    resultRows.push(['Short URL', shareResult.fullShareUrl]);
  }

  if (configResponse && configResponse.success) {
    result(true, t('publish.success'), resultRows);
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, appType, formUuid: savedFormUuid, version, url: pageUrl, appAdminUrl, adminUrl, shareUrl: shareResult.fullShareUrl || null, shareWarning: shareResult.warning || null },
      shareResult.fullShareUrl || pageUrl,
    ...
    result(false, t('publish.config_failed', configErrorMsg), resultRows);
    hint(t('publish.schema_ok_config_failed'));
    console.log(JSON.stringify(withBrowserHandoff(
      { success: true, appType, formUuid: savedFormUuid, version, url: pageUrl, appAdminUrl, adminUrl, shareUrl: shareResult.fullShareUrl || null, shareWarning: shareResult.warning || null, configWarning: configErrorMsg },
      shareResult.fullShareUrl || pageUrl,
      { stage: 'publish_page_success', title: savedFormUuid },
      browserOpenMode
    )));
