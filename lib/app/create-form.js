...
"use strict";
/**
 * create-form-page.ts - 宜搭表单页面创建 & 更新工具
 *
 * 支持两种模式：
 *
 * 1. create 模式 - 创建新表单页面：
...
// ── 选项类字段类型 ───────────────────────────────────
const OPTION_FIELD_TYPES = ['RadioField', 'SelectField', 'CheckboxField', 'MultiSelectField', 'CascadeSelectField'];
// ── 支持的字段类型（宜搭后端支持的 componentName）──────────
// 来源：FORM-C87D231B5F7B4D84A5750F9965F911E7BLF2 表单中实际存在的组件
// 参考：https://www.aliwork.com/solutionCenter?tab=plugin（插件中心）
const SUPPORTED_FIELD_TYPES = [
    // ══════════════════════════════════════════════════════════
    // 基础字段（宜搭内置，无需安装插件）
    // ══════════════════════════════════════════════════════════
    'TextField', 'TextareaField', 'NumberField', 'RateField', 'DateField',
    'RadioField', 'SelectField', 'CheckboxField', 'MultiSelectField', 'CascadeSelectField',
    'CascadeDateField', 'EmployeeField', 'DepartmentSelectField',
    'AssociationFormField', 'AssociationQuery',
    'AttachmentField', 'ImageField',
    'AddressField', 'CountrySelectField',
    'TableField', 'SerialNumberField',
    // ══════════════════════════════════════════════════════════
    // 高级字段（宜搭内置，部分功能可能需要开通）
    // ══════════════════════════════════════════════════════════
    'EditorField', 'DigitalSignatureField', 'LocationField',
    // ══════════════════════════════════════════════════════════
    // 插件字段（需要安装对应插件才能使用）
    // ══════════════════════════════════════════════════════════
    // 国际化时区组件插件 → https://www.aliwork.com/solutionCenter?tab=plugin
    'TimeZoneDateField',
    // AI组件插件
    'CC_AIButtonField',
    // 智能财务组件插件（电子签章）
    'CC_PG_ESignField',
    // Teambition 插件
    'CC_TeambitionProjectLinksField', 'CC_TeambitionTaskLinksField',
    // 其他 CC_ 前缀扩展字段
    'CC_MoneyField', 'CC_PhoneNumberField', 'CC_ChineseIdField',
    // 钉钉相关
    'YidaDingtalkAgentField'
];
// ── 接口路径生成 ──────────────────────────────────────
/**
 * 生成宜搭接口请求路径
...
function parseArgs() {
    ...
    if (mode === 'create') {
        if (args.length < 4) {
            console.error((0, i18n_1.t)('create_form.usage_create'));
            console.error((0, i18n_1.t)('create_form.example_create'));
            process.exit(1);
        }
        return {
            mode: 'create',
    ...
    if (mode === 'update') {
        if (args.length < 4) {
            console.error((0, i18n_1.t)('create_form.usage_update'));
            console.error((0, i18n_1.t)('create_form.example_update'));
            process.exit(1);
        }
        return {
            mode: 'update',
    ...
    console.error((0, i18n_1.t)('create_form.usage_label'));
    console.error((0, i18n_1.t)('create_form.usage_create_short'));
    console.error((0, i18n_1.t)('create_form.usage_update_short'));
    console.error((0, i18n_1.t)('create_form.example_label'));
    console.error((0, i18n_1.t)('create_form.example_create'));
    console.error((0, i18n_1.t)('create_form.example_update'));
    process.exit(1);
}
...
function readFieldsDefinition(fieldsJsonOrFile) {
    ...
    if (fieldsJsonOrFile.trimStart().startsWith('[')) {
        ...
        if (!fs.existsSync(resolvedPath)) {
            console.error((0, i18n_1.t)('create_form.fields_file_not_found') + resolvedPath);
            process.exit(1);
        }
    ...
    try {
        ...
        if (Array.isArray(parsed)) {
            ...
            throw new Error((0, i18n_1.t)('create_form.fields_format_invalid'));
        }
        if (!Array.isArray(fields) || fields.length === 0) {
            throw new Error((0, i18n_1.t)('create_form.fields_must_be_array'));
        }
        return { fields, columns };
    }
    catch (parseError) {
        console.error((0, i18n_1.t)('create_form.fields_parse_failed') + parseError.message);
        process.exit(1);
    }
}
// ── 读取修改定义（update 模式） ─────────────────────
function readChangesDefinition(changesJsonOrFile) {
    ...
    if (changesJsonOrFile.trimStart().startsWith('[')) {
        ...
        if (!fs.existsSync(resolvedPath)) {
            console.error((0, i18n_1.t)('create_form.changes_file_not_found') + resolvedPath);
            process.exit(1);
        }
        rawContent = fs.readFileSync(resolvedPath, 'utf-8');
    }
    try {
        ...
        if (!Array.isArray(changes) || changes.length === 0) {
            throw new Error((0, i18n_1.t)('create_form.changes_must_be_array'));
        }
        return changes;
    }
    catch (parseError) {
        console.error((0, i18n_1.t)('create_form.changes_parse_failed') + parseError.message);
        process.exit(1);
    }
}
// ── 自增 ID 计数器 ───────────────────────────────────
...
function buildFieldComponent(field) {
    ...
    if (!SUPPORTED_FIELD_TYPES.includes(componentName)) {
        console.error((0, i18n_1.t)('create_form.unsupported_field_type', componentName, SUPPORTED_FIELD_TYPES.join(', ')));
        process.exit(1);
    }
    ...
    fieldComponents.forEach(function (component) {
        ...
        function resolveRules(rules) {
            ...
            rules.forEach(function (rule) {
                ...
                if (rule.target && typeof rule.target === 'string' && rule.target.startsWith('@label:')) {
                    ...
                    if (resolvedId) {
                        console.error((0, i18n_1.t)('create_form.filling_rule_resolved', targetLabel, resolvedId));
                        rule.target = resolvedId;
                    }
                    else {
                        console.error((0, i18n_1.t)('create_form.filling_rule_failed', targetLabel));
                    }
                }
                ...
                if (rule.source && typeof rule.source === 'string' && rule.source.startsWith('@label:')) {
                    ...
                    if (resolvedSourceId) {
                        console.error((0, i18n_1.t)('create_form.filling_rule_resolved', sourceLabel, resolvedSourceId));
                        rule.source = resolvedSourceId;
                    }
                    else {
                        console.error((0, i18n_1.t)('create_form.filling_rule_failed', sourceLabel));
                    }
                }
            });
        }
        ...
        function resolveTableRules(tableRules) {
            ...
            tableRules.forEach(function (tableRule, tableIndex) {
                ...
                console.error((0, i18n_1.t)('create_form.table_filling_rule', String(tableIndex + 1), tableRule.tableId));
                tableRule.rules.forEach(function (rule, ruleIndex) {
                    if (rule.target && typeof rule.target === 'string' && rule.target.startsWith('@label:')) {
                        const targetLabel = rule.target.slice(7);
                        const resolvedId = labelToFieldId[targetLabel];
                        if (resolvedId) {
                            console.error((0, i18n_1.t)('create_form.table_rule_resolved', String(ruleIndex + 1), targetLabel, resolvedId));
                            rule.target = resolvedId;
                        }
                        else {
                            console.error((0, i18n_1.t)('create_form.table_rule_failed', targetLabel));
                        }
                    }
                });
            });
            ...
            dataSource: {
                ...
                globalConfig: {
                    fit: {
                        ...
                        source: "function fit(response) {\r\n  const content = (response.content !== undefined) ? response.content : response;\r\n  const error = {\r\n    message: response.errorMsg ||\r\n      (response.errors && response.errors[0] && response.errors[0].msg) ||\r\n      response.content || '远程数据源请求出错，success is false',\r\n  };\r\n  let success = true;\r\n  if (response.success !== undefined) {\r\n    success = response.success;\r\n  } else if (response.hasError !== undefined) {\r\n    success = !response.hasError;\r  }\r  return {\n    content,\n    success,\n    error,\n  };\n}",
                        type: 'js',
                        error: {},
                    },
                },
                online: (0, datasource_utils_1.buildDataSourceList)(datasourceDefinitions || [], formUuid, csrfToken),
                list: (0, datasource_utils_1.buildDataSourceList)(datasourceDefinitions || [], formUuid, csrfToken),
                sync: true,
            },
            lifeCycles: {
                constructor: {
...
function applyChangesToSchema(schema, changes) {
    ...
    if (!componentsTree || componentsTree.length === 0) {
        console.error((0, i18n_1.t)('create_form.no_components_tree'));
        process.exit(1);
    }
    const formContainer = findFormContainer(componentsTree[0]);
    if (!formContainer) {
        console.error((0, i18n_1.t)('create_form.no_form_container'));
        process.exit(1);
    }
    const formFields = formContainer.children || [];
    const appliedChanges = [];
    changes.forEach(function (change, changeIndex) {
        const actionDesc = (0, i18n_1.t)('create_form.action_label', String(changeIndex + 1), change.action);
        if (change.action === 'add') {
            if (!change.field || !change.field.type || !change.field.label) {
                console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.add_missing_field'));
                return;
            }
            ...
            if (change.after) {
                ...
                if (afterIndex !== -1) {
                    ...
                    console.error('  ✅ ' + actionDesc + (0, i18n_1.t)('create_form.add_after_ok', change.after, change.field.label, change.field.type));
                }
                else {
                    formFields.push(newComponent);
                    console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.add_after_not_found', change.after, change.field.label));
                }
            }
                ...
                if (beforeIndex !== -1) {
                    ...
                    console.error('  ✅ ' + actionDesc + (0, i18n_1.t)('create_form.add_before_ok', change.before, change.field.label, change.field.type));
                }
                else {
                    formFields.push(newComponent);
                    console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.add_before_not_found', change.before, change.field.label));
                }
            }
            else {
                formFields.push(newComponent);
                console.error('  ✅ ' + actionDesc + (0, i18n_1.t)('create_form.add_ok', change.field.label, change.field.type));
            }
            appliedChanges.push({ action: 'add', label: change.field.label, type: change.field.type });
        }
        else if (change.action === 'delete') {
            if (!change.label) {
                console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.delete_missing_label'));
                return;
            }
            const deleteIndex = findFieldIndexByLabel(formFields, change.label);
            if (deleteIndex !== -1) {
                formFields.splice(deleteIndex, 1);
                console.error('  ✅ ' + actionDesc + (0, i18n_1.t)('create_form.delete_ok', change.label));
                appliedChanges.push({ action: 'delete', label: change.label });
            }
            else {
                console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.delete_not_found', change.label));
            }
        }
        else if (change.action === 'update') {
            if (!change.label) {
                console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.update_missing_label'));
                return;
            }
            if (!change.changes || Object.keys(change.changes).length === 0) {
                console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.update_missing_changes'));
                return;
            }
            ...
            if (change.tableLabel) {
                ...
                if (tableIndex === -1) {
                    console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.update_table_not_found', change.tableLabel));
                    return;
                }
                const tableComponent = formFields[tableIndex];
                if (tableComponent.componentName !== 'TableField' || !tableComponent.children) {
                    console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.update_not_table', change.tableLabel));
                    return;
                }
                searchFields = tableComponent.children;
                locationDesc = (0, i18n_1.t)('create_form.in_table', change.tableLabel);
            }
            ...
            if (updateIndex !== -1) {
                ...
                console.error('  ✅ ' + actionDesc + (0, i18n_1.t)('create_form.update_ok', locationDesc, change.label, changedProps));
                appliedChanges.push({ action: 'update', label: change.label, tableLabel: change.tableLabel || null, changedProps: changedProps });
            }
            else {
                console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.update_not_found', locationDesc, change.label));
            }
        }
        else {
            console.error('  ⚠️ ' + actionDesc + (0, i18n_1.t)('create_form.unknown_action', change.action));
        }
    });
...
function sendPostRequest(baseUrl, csrfToken, cookies, requestPath, extraParams, formUuid) {
    return new Promise((resolve, reject) => {
        const postData = querystring.stringify(Object.assign({ _csrf_token: csrfToken }, extraParams));
        const parsedUrl = new URL(baseUrl);
        const requestHost = parsedUrl.hostname;
        const filteredCookies = cookies.filter(c => {
            const cookieDomain = (c.domain || '').replace(/^\./, '');
...
function resolveCorpId(cookieData) {
    // 优先使用已提取的 corp_id 字段
    if (cookieData.corp_id) {
        return cookieData.corp_id;
    }
...
async function saveSchemaAndUpdateConfig(authRef, appType, formUuid, schema, version, stepOffset) {
    ...
    console.error((0, i18n_1.t)('create_form.step_save_schema', String(saveStep)));
    console.error((0, i18n_1.t)('create_form.sending_save'));
    const saveResult = await requestWithAutoLogin(function (auth) {
        return sendPostRequest(auth.baseUrl, auth.csrfToken, auth.cookies, buildApiPath(appType, 'saveFormSchema', { prefix: '_view' }), { appType: appType, formUuid: formUuid, content: JSON.stringify(schema), schemaVersion: 'V5', prefix: '_view' }, formUuid);
    }, authRef);
    if (!saveResult || !saveResult.success) {
        const saveErrorMsg = saveResult ? saveResult.errorMsg || (0, i18n_1.t)('common.unknown_error') : (0, i18n_1.t)('common.request_failed');
        console.error((0, i18n_1.t)('create_form.save_schema_failed', saveErrorMsg));
        if (saveResult && !saveResult.__needLogin) {
            console.error((0, i18n_1.t)('common.response_detail', JSON.stringify(saveResult, null, 2)));
        }
        console.error('='.repeat(50));
        console.log(JSON.stringify({ success: false, formUuid: formUuid, error: saveErrorMsg }));
        process.exit(1);
    }
    console.error((0, i18n_1.t)('create_form.schema_saved'));
    if (version !== undefined) {
        console.error((0, i18n_1.t)('create_form.current_version', String(version)));
    }
    console.error((0, i18n_1.t)('create_form.step_update_config', String(configStep)));
    console.error((0, i18n_1.t)('create_form.sending_config'));
...
async function mainCreate(parsedArgs, csrfToken, cookies, baseUrl, cookieData) {
    ...
    const SEP = '='.repeat(50);
    console.error(SEP);
    console.error((0, i18n_1.t)('create_form.create_title'));
    console.error(SEP);
    console.error((0, i18n_1.t)('create_form.app_id', appType));
    console.error((0, i18n_1.t)('create_form.form_title', formTitle));
    console.error((0, i18n_1.t)('create_form.fields_def', fieldsJsonOrFile));
    ...
    // Step 2: 读取字段定义
    console.error((0, i18n_1.t)('create_form.step_read_fields', '2'));
    const { fields, columns } = readFieldsDefinition(fieldsJsonOrFile);
    console.error((0, i18n_1.t)('create_form.fields_loaded', fields.length.toString()));
    console.error((0, i18n_1.t)('create_form.pc_columns', String(columns)));
    fields.forEach(function (field, index) {
        console.error('     ' + (index + 1) + '. ' + field.type + ': ' + field.label);
    });
    // Step 3: 创建空白表单
    console.error((0, i18n_1.t)('create_form.step_create_blank', '3'));
    console.error((0, i18n_1.t)('create_form.sending_create'));
    const createResult = await requestWithAutoLogin(function (auth) {
        return sendPostRequest(auth.baseUrl, auth.csrfToken, auth.cookies, buildApiPath(appType, 'saveFormSchemaInfo'), { formType: 'receipt', title: JSON.stringify(i18n(formTitle)) });
    }, authRef);
    if (!createResult || !createResult.success || !createResult.content) {
        const errorMsg = createResult ? createResult.errorMsg || (0, i18n_1.t)('common.unknown_error') : (0, i18n_1.t)('common.request_failed');
        console.error((0, i18n_1.t)('create_form.create_blank_failed', errorMsg));
        console.log(JSON.stringify({ success: false, error: errorMsg }));
        process.exit(1);
    }
    const formUuid = createResult.content.formUuid || createResult.content;
    console.error((0, i18n_1.t)('create_form.blank_created', formUuid));
    // Step 4 & 5: 生成 Schema 并保存，然后更新表单配置
    const corpId = resolveCorpId(authRef.cookieData);
    if (!corpId) {
        console.error((0, i18n_1.t)('create_form.no_corp_id_warning'));
    }
    else {
        console.error((0, i18n_1.t)('create_form.corp_id_ok', corpId));
    }
    const datasourceDefinitions = (0, datasource_utils_1.parseDatasourceArg)(datasource || null);
    if (datasourceDefinitions.length > 0) {
        console.error((0, i18n_1.t)('create_form.datasource_count', datasourceDefinitions.length.toString()));
        // 自动查询鉴权账号名称，填充 connectionLabel（有 connectionId 但缺少 connectionLabel 时）
        await (0, datasource_utils_1.enrichDatasourceDefinitionsWithConnectionLabels)(datasourceDefinitions, authRef.baseUrl, authRef.csrfToken, authRef.cookies);
    }
    ...
    if (configResult && configResult.success) {
        console.error((0, i18n_1.t)('create_form.create_success'));
        console.error((0, i18n_1.t)('create_form.form_uuid_label', formUuid));
        console.error((0, i18n_1.t)('create_form.url_label', formUrl));
        console.error((0, i18n_1.t)('create_form.config_updated_0'));
        console.error(SEP2);
        ...
        console.error((0, i18n_1.t)('create_form.config_failed', errorMsg));
        console.error((0, i18n_1.t)('create_form.schema_ok_config_failed'));
        console.error((0, i18n_1.t)('create_form.form_uuid_label', formUuid));
        console.error((0, i18n_1.t)('create_form.url_label', formUrl));
        if (configResult && !configResult.__needLogin) {
            console.error((0, i18n_1.t)('common.response_detail', JSON.stringify(configResult, null, 2)));
        }
        console.error(SEP2);
        ...
        if (component.componentName === 'SerialNumberField' && component.props) {
            ...
            if (!hasValidFormula) {
                ...
                if (serialNumberRule) {
                    ...
                    console.error((0, i18n_1.t)('create_form.serial_number_formula_set', component.props.label && component.props.label.zh_CN || fieldId));
                }
            }
        }
        // 递归处理子表内的字段
...
async function mainUpdate(parsedArgs, csrfToken, cookies, baseUrl, cookieData) {
    ...
    const SEP = '='.repeat(50);
    console.error(SEP);
    console.error((0, i18n_1.t)('create_form.update_title'));
    console.error(SEP);
    console.error((0, i18n_1.t)('create_form.app_id', appType));
    console.error((0, i18n_1.t)('create_form.form_uuid_input', formUuid));
    console.error((0, i18n_1.t)('create_form.changes_def', changesJsonOrFile));
    // 登录态引用对象，供 requestWithAutoLogin 原地更新
    const authRef = { csrfToken: csrfToken, cookies: cookies, baseUrl: baseUrl, cookieData: cookieData };
    // Step 2: 获取当前表单 Schema
    console.error((0, i18n_1.t)('create_form.step_get_schema', '2'));
    console.error((0, i18n_1.t)('create_form.sending_get_schema'));
    const schemaResult = await requestWithAutoLogin(function (auth) {
        return sendGetRequest(auth.baseUrl, auth.cookies, buildApiPath(appType, 'getFormSchema', { prefix: '_view', namespace: 'alibaba' }), { formUuid: formUuid, schemaVersion: 'V5' });
    }, authRef);
    if (!schemaResult || schemaResult.success === false || schemaResult.__needLogin) {
        const errorMsg = schemaResult ? schemaResult.errorMsg || (0, i18n_1.t)('common.unknown_error') : (0, i18n_1.t)('common.request_failed');
        console.error((0, i18n_1.t)('create_form.get_schema_failed', errorMsg));
        console.log(JSON.stringify({ success: false, error: errorMsg }));
        process.exit(1);
    }
    ...
    if (schemaResult.content) {
        ...
        console.error((0, i18n_1.t)('create_form.schema_extract_failed'));
        console.error((0, i18n_1.t)('create_form.schema_response_structure', JSON.stringify(Object.keys(schemaResult))));
        console.log(JSON.stringify({ success: false, error: (0, i18n_1.t)('create_form.schema_parse_failed') }));
        process.exit(1);
    }
    if (!schema.pages || !Array.isArray(schema.pages) || schema.pages.length === 0) {
        console.error((0, i18n_1.t)('create_form.schema_empty_init'));
    ...
    if (formContainer && formContainer.children) {
        console.error((0, i18n_1.t)('create_form.schema_got_fields', formContainer.children.length));
        ...
        console.error((0, i18n_1.t)('create_form.schema_got_empty'));
    }
    // Step 3: 读取修改定义
    console.error((0, i18n_1.t)('create_form.step_read_changes', '3'));
    const changes = readChangesDefinition(changesJsonOrFile);
    console.error((0, i18n_1.t)('create_form.changes_loaded', changes.length.toString()));
    changes.forEach(function (change, changeIndex) {
        if (change.action === 'add') {
            console.error('     ' + (changeIndex + 1) + '. [' + (0, i18n_1.t)('create_form.action_add') + '] ' + change.field.type + ': ' + change.field.label);
        }
        else if (change.action === 'delete') {
            console.error('     ' + (changeIndex + 1) + '. [' + (0, i18n_1.t)('create_form.action_delete') + '] ' + change.label);
        }
        else if (change.action === 'update') {
            console.error('     ' + (changeIndex + 1) + '. [' + (0, i18n_1.t)('create_form.action_update') + '] ' + change.label + ' → ' + Object.keys(change.changes || {}).join(', '));
        }
    });
    // Step 4: 应用修改
    console.error((0, i18n_1.t)('create_form.step_apply_changes', '4'));
    ...
    const formUrl = authRef.baseUrl + '/' + appType + '/workbench/' + formUuid;
    if (configResult && configResult.success) {
        console.error((0, i18n_1.t)('create_form.update_success'));
        console.error((0, i18n_1.t)('create_form.form_uuid_label', formUuid));
        console.error((0, i18n_1.t)('create_form.url_label', formUrl));
        console.error((0, i18n_1.t)('create_form.changes_applied', appliedChanges.length.toString()));
        console.error((0, i18n_1.t)('create_form.config_updated_0'));
        console.error(SEP2);
