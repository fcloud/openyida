"use strict";
/**
 * save-permission.ts - 宜搭表单权限配置保存命令
 *
 * 用法：openyida save-permission <appType> <formUuid> [选项]
 */
...
function parseArgs(args) {
    if (args.length < 2) {
        console.error('用法: openyida save-permission <appType> <formUuid> [选项]');
...
function validateDataPermission(dataPermission) {
    ...
    if (dataPermission.dataRange && !validRanges.includes(dataPermission.dataRange)) {
        throw new Error(`无效的 dataRange: ${dataPermission.dataRange}，有效值: ${validRanges.join(', ')}`);
    }
}
function validateActionPermission(actionPermission) {
    if (!actionPermission.operations || typeof actionPermission.operations !== 'object') {
        throw new Error('操作权限必须包含 operations 对象，格式为 {"OPERATE_VIEW": true, "OPERATE_EDIT": false, ...}');
    }
    for (const key of Object.keys(actionPermission.operations)) {
        if (!VALID_OPERATE_KEYS.includes(key)) {
            throw new Error(`无效的操作权限 key: ${key}，有效值: ${VALID_OPERATE_KEYS.join(', ')}`);
        }
    }
}
/**
...
function savePermitPackage(appType, formUuid, permitPackage, overrideMembers, authRef) {
    return new Promise((resolve) => {
        ...
        const postData = querystring_1.default.stringify(postParams);
        const options = {
            hostname: parsedUrl.hostname,
            port: parsedUrl.port || (isHttps ? 443 : 80),
            path: requestPath,
...
async function run(args) {
    ...
    if (createMode) {
        ...
        if (members) {
            console.error(`  → 成员: ${members.join(', ')}`);
        }
        const createResult = await (0, utils_1.requestWithAutoLogin)((auth) => savePermitPackage(appType, formUuid, newPkg, null, auth), authRef);
        console.error('\n' + SEP);
        if (createResult && createResult.success) {
            ...
            console.error(`  ❌ 新增失败: ${createResult && createResult.errorMsg || (0, i18n_1.t)('common.unknown_error')}`);
            console.error(SEP);
            process.exit(1);
        }
        return;
    ...
    // ── 更新已有权限组模式 ──────────────────────────────────────────────────────
    console.error('\n📋 Step 2: 获取当前权限组列表');
    const listResult = await (0, utils_1.requestWithAutoLogin)((auth) => fetchPermitPackages(appType, formUuid, auth), authRef);
    if (!listResult || !listResult.success) {
        console.error(`  ❌ 获取权限组失败: ${listResult && listResult.errorMsg || (0, i18n_1.t)('common.unknown_error')}`);
        process.exit(1);
    }
    ...
    for (const pkg of packagesToUpdate) {
        ...
        const saveResult = await (0, utils_1.requestWithAutoLogin)((auth) => savePermitPackage(appType, formUuid, updatedPkg, overrideMembers, auth), authRef);
        if (saveResult && saveResult.success) {
            console.error('    ✅ 更新成功');
        }
        else {
            console.error(`    ❌ 更新失败: ${saveResult && saveResult.errorMsg || (0, i18n_1.t)('common.unknown_error')}`);
            allSuccess = false;
        }
    }
    console.error('\n' + SEP);
    if (allSuccess) {
        ...
        const summary = {};
        if (dataPermission) {
            summary.dataPermission = `数据范围: ${dataPermission.dataRange}`;
        }
        if (actionPermission) {
            summary.actionPermission = `操作权限: ${Object.keys(actionPermission.operations).join(', ')}`;
        }
        if (members) {
            summary.members = `成员: ${members.join(', ')}`;
        }
...
//# sourceMappingURL=save-permission.js.map