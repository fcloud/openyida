...
async function fetchDingtalkOAuthQrCodeUrl(loginPageUrl, cookieHeader, options = {}) {
  ...
  const rawQrUrl = parsed.result || parsed.content || parsed.qrUrl;
  if (!parsed.success || !rawQrUrl) {
    throw new Error(t('qr_login.get_qr_api_failed', parsed.errorMsg || JSON.stringify(parsed)));
  }

  // For Global YiDA (yidaapps.com), generate_qrcode may return a dingtalk.com
  // QR URL even when called against dingtalk.io. Patch it so Global DingTalk
  // users can scan the QR code.
  const qrUrl = patchGlobalDingtalkDomain(rawQrUrl, options.baseUrl);
...
async function fetchQrCodeUrl(baseUrl, cookieHeader, loginPageUrl, options = {}) {
  if (isDingtalkOAuthChallengeUrl(loginPageUrl)) {
    return fetchDingtalkOAuthQrCodeUrl(loginPageUrl, cookieHeader, { ...options, baseUrl });
  }
