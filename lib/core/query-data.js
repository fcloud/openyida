 ...
 *
 * 用法：openyida data <action> <resource> <appType> [formUuid] [options]
 *
 * USAGE:
...
const { loadCookieData, resolveBaseUrl, httpGet, httpPost, requestWithAutoLogin } = require('./utils');

/**
 * 将 kebab-case 字符串转为 snake_case。
 */
...
async function run(args) {
    ...
    if (action === 'query' && resource === 'form') {
        ...
        if (options.search_json) {
            queryParams.searchFieldJson = options.search_json;
        }
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/form/searchFormDatas.json`, queryParams, auth.cookies),
            authRef
        );
        ...
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/form/getFormDataById.json`, queryParams, auth.cookies),
            authRef
        );
    } else if (action === 'create' && resource === 'form') {
        result = await requestWithAutoLogin(
            (auth) => httpPost(auth.baseUrl, `/dingtalk/web/${appType}/v1/form/saveFormData.json`, {
        ...
        result = await requestWithAutoLogin(
            (auth) => httpPost(auth.baseUrl, `/dingtalk/web/${appType}/v1/form/updateFormData.json`, {
                _csrf_token: auth.csrfToken,
                formInstId: options.inst_id,
                appType,
        ...
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/form/getSubFormDatas.json`, {
                _csrf_token: auth.csrfToken,
                _stamp: String(Date.now()),
                formInstId: options.inst_id,
                appType,
        ...
        clampPageSize(options);
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/process/getInstances.json`, {
                _csrf_token: auth.csrfToken,
                _stamp: String(Date.now()),
                formUuid,
                appType,
        ...
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/process/getInstanceById.json`, {
                _csrf_token: auth.csrfToken,
                _stamp: String(Date.now()),
        ...
        result = await requestWithAutoLogin(
            (auth) => httpPost(auth.baseUrl, `/dingtalk/web/${appType}/v1/process/startInstance.json`, {
                _csrf_token: auth.csrfToken,
                formUuid,
                appType,
                formDataJson: options.data || '{}',
        ...
        result = await requestWithAutoLogin(
            (auth) => httpPost(auth.baseUrl, `/dingtalk/web/${appType}/v1/process/updateInstance.json`, {
                _csrf_token: auth.csrfToken,
                processInstanceId: formUuid,
                appType,
                updateFormDataJson: options.data || '{}',
        ...
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/process/getOperationRecords.json`, {
                _csrf_token: auth.csrfToken,
                _stamp: String(Date.now()),
                processInstanceId: formUuid,
                appType,
        ...
        result = await requestWithAutoLogin(
            (auth) => httpPost(auth.baseUrl, `/dingtalk/web/${appType}/v1/task/executeTask.json`, {
                _csrf_token: auth.csrfToken,
                taskId: formUuid,
                appType,
                formDataJson: options.data || '{}',
        ...
        clampPageSize(options);
        result = await requestWithAutoLogin(
            (auth) => httpGet(auth.baseUrl, `/dingtalk/web/${appType}/v1/task/getTasks.json`, {
                _csrf_token: auth.csrfToken,
                _stamp: String(Date.now()),
                appType,
                currentPage: String(options.page),
    ...
    if (result && result.success) {
        ...
        const errorMsg = (result && result.errorMsg) ? result.errorMsg : '未知错误';
