...
function buildSearchFormDatasRequest(appType, formUuid, options) {
    return (authRef) => {
        ...
        return (0, utils_1.httpGet)(authRef.baseUrl, requestPath, queryParams, authRef.cookies);
    };
}
...
async function run(args) {
    ...
    if (options.instId) {
        ...
        result = await (0, utils_1.requestWithAutoLogin)(buildGetFormDataByIdRequest(appType, options.instId), authRef);
    }
    else {
        console.error(`  查询数据列表（第 ${options.page} 页，每页 ${options.size} 条）...`);
        result = await (0, utils_1.requestWithAutoLogin)(buildSearchFormDatasRequest(appType, formUuid, options), authRef);
    }
    console.error('\n' + SEP);
