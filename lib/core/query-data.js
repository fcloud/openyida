...
async function sendGet(session, appType, requestPath, params) {
    return (0, utils_1.requestWithAutoLogin)((auth) => (0, utils_1.httpGet)(auth.baseUrl, requestPath, buildRequestParams(auth, params), auth.cookies), session);
}
async function sendPost(session, appType, requestPath, params) {
    return (0, utils_1.requestWithAutoLogin)((auth) => (0, utils_1.httpPost)(auth.baseUrl, requestPath, querystring.stringify(buildRequestParams(auth, params)), auth.cookies), session);
}
...
async function queryForm(positionals, options, session) {
    ...
    if (options.inst_id) {
        ...
        for (const key of ['originator_id', 'create_from', 'create_to', 'modified_from', 'modified_to', 'dynamic_order']) {
            if (options[key]) {
                params[snakeToCamel(key)] = options[key];
            }
        }
 ...
 */
function extractLabelToFieldIdMap(schema) {
    const labelMap = {};
 ...
 * 将用户数据中的字段标签替换为字段 ID
 */
function convertLabelToFieldId(data, labelMap) {
    const result = {};
...
async function createForm(positionals, options, session) {
    ...
    let formDataJson = options.data;
    if (hasChineseKey) {
        // 获取表单 Schema 以提取字段映射
        console.error('  📋 检测到字段标签，正在获取字段映射...');
        const schemaResult = await sendGet(session, appType, `/alibaba/web/${appType}/_view/query/formdesign/getFormSchema.json`, { formUuid });
...
async function queryProcess(positionals, options, session) {
    ...
    for (const key of ['search_json', 'task_id', 'instance_status', 'approved_result', 'originator_id', 'create_from', 'create_to', 'modified_from', 'modified_to']) {
        if (options[key]) {
            params[key === 'search_json' ? 'searchFieldJson' : snakeToCamel(key)] = options[key];
        }
    }
    const requestPath = options.ids_only
...
async function executeTask(positionals, options, session) {
    ...
    if (options.data) {
        params.formDataJson = options.data;
    }
    if (options.no_execute_expressions) {
...
async function queryTasks(positionals, options, session) {
    ...
    if (options.process_codes) {
        params.processCodes = options.process_codes;
    }
    if (options.instance_status) {
...
async function run(args) {
    ...
    if (action === 'query' && resource === 'form') {
        return queryForm(positionals, options, session);
    }
    if (action === 'get' && resource === 'form') {
        return getForm(positionals, options, session);
    }
    if (action === 'create' && resource === 'form') {
        return createForm(positionals, options, session);
    }
    if (action === 'update' && resource === 'form') {
        return updateForm(positionals, options, session);
    }
    if (action === 'query' && resource === 'subform') {
        return querySubform(positionals, options, session);
    }
    if (action === 'query' && resource === 'process') {
        return queryProcess(positionals, options, session);
    }
    if (action === 'get' && resource === 'process') {
        return getProcess(positionals, options, session);
    }
    if (action === 'create' && resource === 'process') {
        return createProcess(positionals, options, session);
    }
    if (action === 'update' && resource === 'process') {
        return updateProcess(positionals, options, session);
    }
    if (action === 'query' && resource === 'operation-records') {
        return queryOperationRecords(positionals, options, session);
    }
    if (action === 'execute' && resource === 'task') {
        return executeTask(positionals, options, session);
    }
    if (action === 'query' && resource === 'tasks') {
        return queryTasks(positionals, options, session);
    }
