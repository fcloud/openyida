...
function cmdSwitch(envName) {
  ...
  if (!config.environments[resolvedName]) {
    warn(`${RED}错误：环境 "${resolvedName}" 不存在${RESET}`);
    warn(`使用 ${CYAN}openyida env list${RESET} 查看所有环境`);
    process.exit(1);
  }

  if (config.current === resolvedName) {
    console.log(`${YELLOW}当前已在 "${resolvedName}" 环境，无需切换${RESET}`);
    return;
  }

  const previousEnv = config.current;
  config.current = resolvedName;
  saveEnvsConfig(config);

  console.log('');
  console.log(`${GREEN}✅ 已切换环境${RESET}`);
  console.log(`  ${DIM}${previousEnv}${RESET} → ${GREEN}${BOLD}${resolvedName}${RESET}`);
  console.log(`  地址：${config.environments[resolvedName].baseUrl}`);
  console.log('');
