...
const DEFAULT_GLOBAL_BASE_URL = 'https://www.yidaapps.com';
const DEFAULT_GLOBAL_LOGIN_URL = 'https://www.yidaapps.com/workPlatform';
...
/** Global YiDA environment (yidaapps.com) */
const DEFAULT_GLOBAL_ENV = {
  baseUrl: DEFAULT_GLOBAL_BASE_URL,
  loginUrl: DEFAULT_GLOBAL_LOGIN_URL,
  description: 'Global YiDA (yidaapps.com)',
  cookieFile: 'cookies-global.json',
};

const BUILTIN_ENVIRONMENTS = {
  public: DEFAULT_PUBLIC_ENV,
  alibaba: DEFAULT_ALIBABA_INTERNAL_ENV,
  global: DEFAULT_GLOBAL_ENV,
};
...
/**
 * Resolve a user-supplied environment name or natural-language alias to a
 * canonical environment key.  Lookup is case-insensitive and accent-folded.
 * Returns the original input unchanged when no alias matches, so the caller
 * can still surface a "not found" error with the original string.
 *
 * @param {string} input
 * @returns {string}
 */
function resolveEnvName(input) {
  ...
  return ENV_ALIASES[normalized] || input.trim();
}
