...
/**
 * 触发登录。AI 工具环境优先使用 CDP，其他环境使用本地浏览器扫码模式。
 * @returns {object} loginResult
 */
function triggerLogin() {
  warn(t('login.trigger_login'));
  const activeTool = detectActiveTool();
  const cdpDisabled = ['1', 'true', 'yes'].includes(String(process.env.OPENYIDA_DISABLE_CDP_LOGIN || '').toLowerCase());
  if (!cdpDisabled && activeTool && ['codex', 'qoder', 'wukong'].includes(activeTool.tool)) {
    const { cdpLogin } = require('../auth/cdp-login');
    const result = cdpLogin();
