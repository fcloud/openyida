...
exports.installSkillsToAllAgents = installSkillsToAllAgents;
...
function removePath(targetPath, silent) {
    ...
    try {
        ...
        if (!silent) {
            const err = error;
            console.error((0, i18n_1.t)('copy.remove_failed', targetPath, err.message));
        }
        return false;
    }
}
/**
 * 将 yida-skills 安装（软链接）到所有指定的 AI 工具目录。
 * 悟空工具（dirName === '.real'）只清理已有软链，不创建新软链。
 */
function installSkillsToAllAgents(sourceDir, agents) {
