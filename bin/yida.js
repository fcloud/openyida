 ...
 *   openyida create-report <appType> "<报表名>" <数据源formUuid> <报表定义JSON>  创建原生报表
...
命令：
  ...
  create-report <appType> "<报表名>" <formUuid> <报表定义JSON>  创建原生报表
...
async function main() {
  ...
  switch (command) {
    ...
    case 'create-report': {
      ...
      const { run: runCreateReport } = require('../lib/create-report');
      await runCreateReport(args);
      break;
    }
